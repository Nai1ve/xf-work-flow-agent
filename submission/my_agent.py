...
class SlotResolver:
    """Resolve general language slots before collect_input decides to block."""

    def __init__(self, agent: Any):
        self.agent = agent

    def resolve(self, state: "RuntimeState", runtime: "TaskRuntime") -> None:
        self.agent._activate_task_runtime_view(state, runtime)
        source = str(runtime.task.get("source_text") or runtime.slots.get("source_text") or self.agent._full_query(state.obs))
        if runtime.domain == "meetingroom":
            inferred = self.agent._heuristic_meetingroom(source, state.obs)
            self._fill_missing(runtime.slots, inferred)
            state.meetingroom.slots = runtime.slots
            self.agent._normalize_meeting_slots(state)
            runtime.slots = state.meetingroom.slots
            return
        if runtime.intent == "leave":
            leave = runtime.slots.setdefault("leave", {})
            inferred = self.agent._heuristic_leave(source)
            self._fill_missing(leave, inferred)
            day_text = str(leave.get("day_text") or "")
            duration = leave.get("duration_hours") or self.agent._extract_duration_hours(source)
            explicit_clock_range = bool(
                re.search(r"(?:\d{1,2}(?::\d{2})?|[一二三四五六七八九十两]+)\s*点?.{0,8}(?:到|至|-|~).{0,8}\d{1,2}(?::\d{2})?\s*点?", source)
            )
            if duration and leave.get("start") and not explicit_clock_range:
                try:
                    leave["end"] = self.agent._add_minutes(str(leave["start"]), int(float(duration) * 60))
                except (TypeError, ValueError):
                    pass
            if any(token in day_text for token in ["到", "至", "-", "~"]):
                leave.setdefault("start", "09:00")
                leave.setdefault("end", "18:00")
            state.workflow.slots = runtime.slots
            self.agent._normalize_workflow_slots(state)
            start_day, end_day = self.agent._resolve_leave_date_range(day_text, state)
            if start_day:
                leave["day"] = start_day
                leave["end_day"] = end_day or start_day
            runtime.slots = state.workflow.slots
            return
        if runtime.intent == "expense_material":
            expense = runtime.slots.setdefault("expense", {})
            self._fill_missing(expense, self.agent._heuristic_expense(source))
            state.workflow.slots = runtime.slots
            self.agent._normalize_workflow_slots(state)
            runtime.slots = state.workflow.slots

    @staticmethod
    def _fill_missing(target: dict[str, Any], incoming: dict[str, Any]) -> None:
...
class StepAction:
    def __init__(self, kind: str, tool: str = "", args: dict[str, Any] | None = None, message: str = ""):
        ...
        self.read_options: dict[str, Any] = {}
...
class ToolExecutionContext:
    def __init__(
        ...
        *,
        ...
        self.owner_task_id = str(owner_task_id or "")
        self.owner_node_id = str(owner_node_id or "")
...
class ReadTask:
    def __init__(
        ...
        for owner in owners or []:
            if isinstance(owner, dict):
                self.add_owner(
                    str(owner.get("task_id") or owner.get("owner_task_id") or ""),
                    str(owner.get("node_id") or owner.get("owner_node_id") or ""),
                    str(owner.get("domain") or domain or ""),
                    owner.get("metadata") if isinstance(owner.get("metadata"), dict) else {},
                )
        if not self.owners:
            self.add_owner(owner_task_id, owner_node_id, domain)

    @property
    def owner_task_id(self) -> str:
        return str((self.owners[0] if self.owners else {}).get("task_id") or "")

    @property
    def owner_node_id(self) -> str:
        return str((self.owners[0] if self.owners else {}).get("node_id") or "")
    ...
    def add_owner(
        ...
        owner = {
            ...
            "domain": str(domain or self.domain or ""),
            "metadata": dict(metadata or {}),
        }
        if not owner["task_id"] and not owner["node_id"]:
            return
...
class ReadPlan:
    ...
    def ready_tasks(
        ...
        completed: set[str],
        limit: int,
        completed_owners: set[str] | None = None,
    ) -> list[ReadTask]:
        out = []
        seen: set[str] = set()
        completed_owners = completed_owners or set()
        for task in self.tasks:
            owner_keys = {
                f"{task.task_key}|{item.get('task_id') or ''}|{item.get('node_id') or ''}"
                for item in task.owners
            }
            if (owner_keys and owner_keys.issubset(completed_owners)) or task.task_key in seen:
                continue
...
class BusinessSkillRegistry:
    ...
    def definition(self, skill_id: str) -> dict[str, Any]:
        ...
        if parent_id:
            ...
            for node in parent.get("nodes") or []:
                ...
                merged_node = json.loads(json.dumps(node, ensure_ascii=False))
                replacement = replacements.get(node_id)
                if isinstance(replacement, dict):
                    self._merge_dict(merged_node, replacement)
    ...
    def validate_contracts(self) -> list[str]:
        ...
        for skill_id in sorted(self.skills):
            ...
            if not ids or any(not node_id for node_id in ids) or len(ids) != len(set(ids)):
                errors.append(f"{skill_id}:invalid_node_ids")
                continue
            id_set = set(ids)
            for node in nodes:
                ...
                if not node.get("success_evidence"):
                    errors.append(f"{skill_id}:{node_id}:missing_success_evidence")
                if int(node.get("cost") or 0) > 0 and not node.get("tool"):
                    errors.append(f"{skill_id}:{node_id}:missing_tool")
                for dependency in node.get("depends_on") or []:
                    if str(dependency) not in id_set:
                        errors.append(f"{skill_id}:{node_id}:unknown_dependency:{dependency}")
                edges = node.get("failure_edges") if isinstance(node.get("failure_edges"), dict) else {}
                if not edges:
                    errors.append(f"{skill_id}:{node_id}:missing_failure_edges")
                for reason, edge in edges.items():
                    if not isinstance(edge, dict) or str(edge.get("action") or "") not in {"retry", "skip", "block"}:
                        errors.append(f"{skill_id}:{node_id}:{reason}:invalid_failure_action")
                        continue
                    if edge.get("action") == "retry":
                        if phase == "write":
                            errors.append(f"{skill_id}:{node_id}:{reason}:write_retry_forbidden")
                        target = str(edge.get("target") or node_id)
                        if target not in id_set:
                            errors.append(f"{skill_id}:{node_id}:{reason}:unknown_retry_target:{target}")
                        if int(edge.get("max_attempts") or 0) < 1:
                            errors.append(f"{skill_id}:{node_id}:{reason}:invalid_max_attempts")
            errors.extend(self._dependency_cycle_errors(skill_id, nodes))
        return errors

    def validate_capability_coverage(self, capabilities_index: dict[str, Any]) -> list[str]:
    ...
    def status(self, capabilities_index: dict[str, Any] | None = None) -> dict[str, Any]:
        contract_errors = self.validate_contracts()
        coverage_errors = self.validate_capability_coverage(capabilities_index or {}) if capabilities_index else []
        return {
    ...
    def _dependency_cycle_errors(self, skill_id: str, nodes: list[dict[str, Any]]) -> list[str]:
        dependencies = {str(node.get("id") or ""): [str(item) for item in node.get("depends_on") or []] for node in nodes}
        ...
        def visit(node_id: str) -> bool:
            ...
            if any(dependency in dependencies and visit(dependency) for dependency in dependencies.get(node_id, [])):
                return True
...
class SkillScheduler:
    ...
    def next_action(self, state: "RuntimeState", llm_config: dict[str, Any]) -> SchedulerDecision:
        self.sync(state, llm_config)
        any_progress = False
        transition_limit = max(
            4,
            sum(len(runtime.skill_run.nodes) for runtime in state.task_runtimes if runtime.skill_run is not None) * 2,
        )
        for _ in range(transition_limit):
                ...
                for node in runtime.skill_run.ready_nodes({"compute", "reply"}):
                    ...
                    for action in directive.actions:
                        if isinstance(action, StepAction) and action.kind == "reply":
                            action.execution_context = ToolExecutionContext(
                                owner_task_id=runtime.task_id,
                                owner_node_id=str(node.get("id") or ""),
                                domain=runtime.domain,
                            )
                            return SchedulerDecision(action, progressed=any_progress)
                    validation = {
                        "status": "pending" if directive.status in {"waiting", "ready"} else directive.status,
                    ...
                    if runtime.skill_run.blocked_reason:
                        ...
                        return SchedulerDecision(
                            StepAction(f"block_{runtime.domain}", args={"reason": runtime.blocked_reason}),
                            progressed=True,
                        )
            if not progressed:
                break
            ...
            # A compute/reply transition may unlock a read node. Return to the
            # outer fixed-point loop so ReadPlan is rebuilt before writes.
            if any(
                runtime.skill_run is not None and runtime.skill_run.ready_nodes({"read", "postcheck"})
                for runtime in state.task_runtimes
                if runtime.status not in TaskRuntime.TERMINAL_STATUSES
            ):
                return SchedulerDecision(progressed=True)

        for runtime in state.task_runtimes:
            if runtime.skill_run is None or runtime.status in TaskRuntime.TERMINAL_STATUSES:
                continue
            self.agent._activate_task_runtime_view(state, runtime)
            for node in runtime.skill_run.ready_nodes({"write"}):
                ...
                if handler is None:
                    ...
                    return SchedulerDecision(
                        StepAction(f"block_{runtime.domain}", args={"reason": "missing_args_handler"}),
                        progressed=True,
                    )
                directive = handler(state, runtime, node)
                ...
                if runtime.skill_run.invocation_status(node_id, invocation_fingerprint):
                    ...
                    return SchedulerDecision(
                        StepAction(f"block_{runtime.domain}", args={"reason": "duplicate_skill_invocation"}),
                        progressed=True,
                    )
                ...
                return SchedulerDecision(action, progressed=any_progress)
        return SchedulerDecision(progressed=any_progress, diagnostic=self.stall_diagnostic(state))

    def settle_exhausted_postchecks(self, state: "RuntimeState", llm_config: dict[str, Any]) -> bool:
        """Finish successful writes when only best-effort postchecks lack a tool step."""
        ...
        for runtime in state.task_runtimes:
            if runtime.status in TaskRuntime.TERMINAL_STATUSES or runtime.skill_run is None:
                continue
            for node in runtime.skill_run.ready_nodes({"postcheck"}):
                node_id = str(node.get("id") or "")
                runtime.skill_run.mark_skipped(
                    node_id,
                    source="step_budget",
                    reason="postcheck_not_run_insufficient_step_budget",
                )
                progressed = True
                self.agent._debug_log(
                    llm_config,
    ...
    def stall_diagnostic(self, state: "RuntimeState") -> dict[str, Any]:
        tasks = []
        for runtime in state.task_runtimes:
            if runtime.status in TaskRuntime.TERMINAL_STATUSES or runtime.skill_run is None:
                continue
            ...
            for node in runtime.skill_run.nodes:
                node_id = str(node.get("id") or "")
                if runtime.skill_run.statuses.get(node_id) in SkillRun.TERMINAL_STATUSES:
                    continue
                ...
                pending.append(
                    {
                        "node_id": node_id,
                        "operation": str(node.get("operation") or node.get("phase") or ""),
                        "status": runtime.skill_run.statuses.get(node_id),
                        "depends_on": dependencies,
                        "incomplete_dependencies": [
                            item for item in dependencies if runtime.skill_run.statuses.get(item) != "completed"
                        ],
                        "validation": runtime.skill_run.validations.get(node_id) or {},
                    }
                )
            tasks.append({"task_id": runtime.task_id, "capability": runtime.capability, "nodes": pending})
        return {
            "reason": "insufficient_step_budget" if state.steps_used >= state.step_budget else "scheduler_stalled",
...
class EffectiveToolRegistry(ToolRegistry):
    """Per-run tool contracts with runtime schemas and static safety metadata."""

    def __init__(self, index: dict[str, Any], reconciliation: dict[str, Any]):
        super().__init__(index)
        self.reconciliation = reconciliation
        self.available_tools = set(reconciliation.get("available_tools") or [])
        self.available_unmapped = set(reconciliation.get("available_unmapped") or [])
        self.disabled_tools = set(reconciliation.get("disabled_tools") or [])
        ...
        status.update(
            {
                ...
                "available_unmapped_count": len(self.available_unmapped),
                "disabled_count": len(self.disabled_tools),
                "schema_changed_count": len(self.reconciliation.get("schema_changed_tools") or []),
            }
        )
        return status
    ...
    def can_execute_read(self, tool: str) -> bool:
        return self.is_available(tool) and self.is_read(tool) and tool not in self.available_unmapped

    def can_execute_write(self, tool: str) -> bool:
        return self.is_available(tool) and self.is_write(tool) and tool not in self.available_unmapped

    def validate_call(self, tool: str, args: dict[str, Any], available_tools: set[str] | None = None) -> dict[str, Any]:
        result = super().validate_call(tool, args, available_tools or self.available_tools)
        errors = list(result.get("errors") or [])
        warnings = list(result.get("warnings") or [])
        spec = self.spec(tool)
        if tool in self.available_unmapped:
            warnings.append("runtime_tool_unmapped")
        if tool in self.disabled_tools:
            errors.append("runtime_tool_disabled")
        properties = spec.get("properties") if isinstance(spec.get("properties"), dict) else {}
        type_errors: list[str] = []
        for key, value in args.items():
            expected = properties.get(key) if isinstance(properties.get(key), dict) else {}
            expected_type = str(expected.get("type") or "")
            if expected_type and not self._matches_type(value, expected_type):
                type_errors.append(f"{key}:{expected_type}")
        if type_errors:
            errors.append("invalid_arg_types")
        result.update(
            {
                "passed": not errors,
                "errors": list(dict.fromkeys(errors)),
                "warnings": list(dict.fromkeys(warnings)),
                "invalid_arg_types": type_errors,
                "contract_source": spec.get("contract_source") or "static",
            }
        )
        return result
...
class ToolContractReconciler:
    def __init__(self, static_registry: ToolRegistry):
        self.static_registry = static_registry

    def reconcile(self, runtime_tools: Any) -> EffectiveToolRegistry:
        runtime_by_name = self._runtime_by_name(runtime_tools)
        static_by_name = self.static_registry.by_name
        available = set(runtime_by_name)
        static_names = set(static_by_name)
        by_name: dict[str, dict[str, Any]] = {}
        changed: list[str] = []

        for name in sorted(static_names | available):
            static_spec = static_by_name.get(name) if isinstance(static_by_name.get(name), dict) else {}
            runtime_spec = runtime_by_name.get(name) if isinstance(runtime_by_name.get(name), dict) else {}
            merged = json.loads(json.dumps(static_spec, ensure_ascii=False)) if static_spec else {"name": name}
            runtime_schema = runtime_spec.get("args_schema") if isinstance(runtime_spec.get("args_schema"), dict) else {}
            if runtime_schema:
                properties = runtime_schema.get("properties") if isinstance(runtime_schema.get("properties"), dict) else {}
                required = [str(item) for item in runtime_schema.get("required") or [] if item]
                previous_contract = {
                    "properties": merged.get("properties") or {},
                    "required_args": merged.get("required_args") or [],
                }
                merged["properties"] = json.loads(json.dumps(properties, ensure_ascii=False))
                merged["required_args"] = required
                merged["args_schema"] = json.loads(json.dumps(runtime_schema, ensure_ascii=False))
                if previous_contract != {"properties": properties, "required_args": required}:
                    changed.append(name)
            if runtime_spec.get("description") not in (None, ""):
                merged["description"] = runtime_spec.get("description")
            merged["available"] = name in available
            merged["mapped"] = name in static_names
            merged["contract_source"] = "runtime+static" if name in static_names and name in available else (
                "runtime_unmapped" if name in available else "static_disabled"
            )
        ...
        required_args = {name: list(spec.get("required_args") or []) for name, spec in by_name.items()}
        index = {
            "schema_version": "effective-tools-v1",
            "by_name": by_name,
            "required_args": required_args,
            "write_tools": sorted(self.static_registry.write_tools & available),
            "high_risk_write_tools": sorted(self.static_registry.high_risk_write_tools & available),
            "counts": {"tools": len(by_name)},
        }
        reconciliation = {
            "available_tools": sorted(available),
            "available_unmapped": sorted(available - static_names),
            "disabled_tools": sorted(static_names - available),
            "schema_changed_tools": sorted(set(changed)),
        }
        return EffectiveToolRegistry(index, reconciliation)

    @staticmethod
    def _runtime_by_name(runtime_tools: Any) -> dict[str, dict[str, Any]]:
        if isinstance(runtime_tools, dict):
            values = runtime_tools.values()
        elif isinstance(runtime_tools, list):
            values = runtime_tools
        else:
            values = []
...
class TaskGraphContractNormalizer:
    ...
    MEETING_QUERY_INTENTS = {"query_booking", "query_workspace", "query_room_schedule"}
    ...
    def normalize(self, value: Any, baseline_value: Any = None, query: str = "") -> dict[str, Any]:
        ...
        for index, raw in enumerate(raw_tasks):
            ...
            if original_requested_intent == "query_workspace" and canonical_intent in self.MEETING_ACTION_INTENTS:
                safe_slots["needs_workspace"] = True
                slot_provenance["needs_workspace"] = "llm_translation"
            if disallowed_slots:
        ...
        tasks = self._coalesce_auxiliary_tasks(tasks, query=query)
    ...
    def _ground_requested_intent(
        ...
        if domain != "meetingroom" or requested_intent not in self.MEETING_QUERY_INTENTS or not fallback:
            return requested_intent
        baseline_intent = str(fallback.get("intent") or "")
        if baseline_intent not in self.MEETING_ACTION_INTENTS:
            return requested_intent
        action_words = ["订", "预订", "取消", "延长", "换大", "换一个", "加入", "加到", "移除"]
        return baseline_intent if any(word in str(query or "") for word in action_words) else requested_intent

    def _coalesce_auxiliary_tasks(self, tasks: list[dict[str, Any]], query: str = "") -> list[dict[str, Any]]:
        removed_ids: set[str] = set()
        replacement_ids: dict[str, str] = {}

        def absorb(target: dict[str, Any], source: dict[str, Any]) -> None:
            target_slots = target.setdefault("slots", {})
            for key, value in (source.get("slots") or {}).items():
                if value not in (None, "", [], {}):
                    target_slots.setdefault(key, value)
            target["source_text"] = " ".join(
                self._string_list([target.get("source_text"), source.get("source_text")])
            ).strip()
            source_id = str(source.get("task_id") or "")
            target_id = str(target.get("task_id") or "")
            if source_id and target_id and source_id != target_id:
                removed_ids.add(source_id)
                replacement_ids[source_id] = target_id

        expense_tasks = [task for task in tasks if task.get("domain") == "workflow" and str(task.get("capability") or "").startswith("workflow.expense_")]
        expense_submit = next((task for task in expense_tasks if task.get("capability") == "workflow.expense_submit"), None)
        expense_draft = next((task for task in expense_tasks if task.get("capability") == "workflow.expense_draft"), None)
        if expense_submit is not None and expense_draft is not None:
            explicit_draft = any(token in query for token in ["草稿", "暂存", "不要提交", "不提交"])
            explicit_submit = any(token in query for token in ["提交", "发起", "送审"])
            if explicit_submit and not explicit_draft:
                absorb(expense_submit, expense_draft)
                expense_submit["submit_intent"] = "submit"
            else:
                absorb(expense_draft, expense_submit)
                expense_draft["submit_intent"] = "draft"

        meeting_tasks = [task for task in tasks if task.get("domain") == "meetingroom"]
        book_targets = [
            task for task in meeting_tasks
            if task.get("capability") in {"meeting.book", "meeting.book_multi_segments", "meeting.schedule_book"}
        ]
        action_targets = [
            task for task in meeting_tasks
            if task.get("capability") in {
                "meeting.cancel", "meeting.extend", "meeting.cancel_rebook", "meeting.rebook_larger",
                "meeting.participant_add", "meeting.participant_remove",
            }
        ]
        for task in list(meeting_tasks):
            capability = str(task.get("capability") or "")
            if capability == "meeting.query_workspace" and book_targets:
                target = book_targets[0]
                target.setdefault("slots", {})["needs_workspace"] = True
                absorb(target, task)
            elif capability == "meeting.query_booking" and action_targets:
                absorb(action_targets[0], task)
            elif capability == "meeting.query_room_schedule" and book_targets:
                target = next((item for item in book_targets if item.get("capability") == "meeting.schedule_book"), book_targets[0])
                if target.get("capability") in {"meeting.book", "meeting.book_multi_segments"}:
                    target["capability"] = "meeting.schedule_book"
                    target["intent"] = "book_by_schedule_analysis"
                absorb(target, task)

        schedule_target = next((task for task in book_targets if task.get("capability") == "meeting.schedule_book"), None)
        if schedule_target is not None:
            for task in book_targets:
                if task is schedule_target or str(task.get("task_id") or "") in removed_ids:
                    continue
                absorb(schedule_target, task)

        if not removed_ids:
            return tasks
        out = [task for task in tasks if str(task.get("task_id") or "") not in removed_ids]
        for task in out:
            for field in ("depends_on", "write_after"):
                rewritten = [replacement_ids.get(str(item), str(item)) for item in task.get(field) or []]
                task[field] = self._dedupe_strings(
                    [item for item in rewritten if item and item != str(task.get("task_id") or "")]
                )
        return out
    ...
    def _filter_slots(self, slots: dict[str, Any], source_text: str, query: str) -> tuple[dict[str, Any], dict[str, str], list[str]]:
        # A task can only claim literals from its own source span. The full
        # query is a fallback for legacy tasks without a validated span.
        evidence_text = self._normalized_text(source_text or query)
...
class ToolAdapter:
    def __init__(self, meetingroom_index: MeetingroomIndex, tool_registry: ToolRegistry | None = None):
    ...
    def adapt(self, tool: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if tool == "workflow.save" and adapted.get("workflow_id") not in (None, ""):
            try:
                ...
                pass
        adapted = {key: value for key, value in adapted.items() if value not in (None, "", [], {})}
        spec = self.tool_registry.spec(tool) if self.tool_registry is not None else {}
        properties = spec.get("properties") if isinstance(spec.get("properties"), dict) else {}
        if properties:
            adapted = {key: value for key, value in adapted.items() if key in properties}
            for key, definition in properties.items():
                if key not in adapted or not isinstance(definition, dict):
                    continue
                adapted[key] = self._coerce_arg(adapted[key], str(definition.get("type") or ""))
        return adapted
...
class ResultProjectionRegistry:
    ...
    def _project_task_results(self, state: "RuntimeState", answer: dict[str, Any]) -> None:
        ...
        meeting_blocked: list[dict[str, Any]] = []
        for capability, result in meeting_results:
            if result.get("status") == "blocked":
                meeting_blocked.append(result)
                continue
            if capability == "meeting.participant_list":
                ...
                query = " ".join(
                    [str(state.obs.get("user_query") or "")]
                    + [str(message.get("content") or "") for message in state.obs.get("messages") or [] if isinstance(message, dict)]
                )
                explicit_order = bool(re.search(r"(?:SEED|BK)-[A-Za-z0-9-]+", query, flags=re.I))
                if capability == "meeting.participant_add" and len(participants) > 1 and not explicit_order:
                    answer["participants_added"] = [
                        {key: participant.get(key) for key in ("user_id", "name") if participant.get(key) not in (None, "")}
                ...
                answer["booking_result"] = result
        if "booking_result" not in answer and meeting_blocked:
            answer["booking_result"] = meeting_blocked[0]
        ...
        if workflow_results:
            successful = [item for item in workflow_results if item.get("status") != "blocked"]
            projected = successful[-1] if successful else workflow_results[0]
            answer["workflow_draft_result"] = projected
            blocked = next((item for item in workflow_results if item.get("status") == "blocked"), None)
            if blocked is not None:
                answer["workflow_result"] = dict(blocked)

    def validate(self, state: "RuntimeState", answer: dict[str, Any]) -> dict[str, str]:
        errors: dict[str, str] = {}
        if state.meetingroom.needed and not any(
            key in answer for key in ("booking_result", "participant_result", "participants", "participants_added")
        ):
    ...
    def _project_meetingroom(self, state: "RuntimeState", answer: dict[str, Any]) -> None:
        ...
        if participant_results and mr.intent in {"participant_add", "participant_remove"}:
            query = " ".join(
                [str(state.obs.get("user_query") or "")]
                + [str(item.get("content") or "") for item in state.obs.get("messages") or [] if isinstance(item, dict)]
            )
            explicit_order = bool(re.search(r"(?:SEED|BK)-[A-Za-z0-9-]+", query, flags=re.I))
            if mr.intent == "participant_add" and len(participant_results) > 1 and not explicit_order:
                answer["participants_added"] = [
                    {key: item.get(key) for key in ("user_id", "name") if item.get(key) not in (None, "")}
                    for item in participant_results
                ]
                return
            order_id = participant_results[0].get("order_id") or mr.slots.get("order_id")
            if mr.evidence.get("extend_done") and mr.intent == "participant_add":
                extend_at = query.find("延长")
                add_at = min((pos for pos in (query.find("加入"), query.find("加到"), query.find("参会人")) if pos >= 0), default=-1)
                status = "extended_and_participant_added" if extend_at >= 0 and add_at >= 0 and extend_at < add_at else "updated"
                answer["booking_result"] = {
                    "status": status,
                    "order_id": order_id,
                    "added_user_id": participant_results[0].get("user_id"),
                    "new_end": (mr.evidence.get("extend_done") or {}).get("end"),
                }
                return
            if len(participant_results) == 1:
                ...
                return
            answer["booking_result"] = {
                "status": "participants_added" if mr.intent == "participant_add" else "participants_removed",
                "order_id": order_id,
...
class PreflightGuard:
    ...
    def _validate_workflow_save(self, state: "RuntimeState", args: dict[str, Any]) -> dict[str, Any]:
        ...
        if state.workflow.evidence.get("file_list"):
            evidence_refs.append("file.list")
        return {
            "passed": not errors,
...
class RuntimeState:
    def __init__(self, obs: dict[str, Any], tools: set[str], step_budget: int):
        ...
        self.semantic_elapsed_seconds = 0.0
        ...
        self.read_tasks_failed = 0
        self.read_tasks_retried = 0
        self.read_retry_exhausted = 0
        self.empty_read_mappings_total = 0
        self.empty_read_retry_tasks_total = 0
        self.read_task_keys_completed: set[str] = set()
        self.read_task_owner_keys_completed: set[str] = set()
        self.read_task_keys_attempted: set[str] = set()
        self.read_task_attempt_counts: dict[str, int] = {}
        self.read_task_outcomes: dict[str, ToolExecutionOutcome] = {}
        self.read_task_groups_succeeded: set[str] = set()
        self.history: list[dict[str, Any]] = []
        self.cache: dict[str, Any] = {}
        self.asked_slots: set[str] = set()
        self.reply_messages_attempted: set[str] = set()
        self.reply_messages_unresolved: set[str] = set()
        ...
        self.scheduler_stalls: list[dict[str, Any]] = []
        self.skill_decision_cache: dict[str, dict[str, Any]] = {}
...
class MyAgent:
    def __init__(self, env):
        ...
        self.static_tool_registry = ToolRegistry(self.static_context.tools)
        self.tool_contract_reconciler = ToolContractReconciler(self.static_tool_registry)
        self.tool_registry: ToolRegistry = self.static_tool_registry
        ...
        self.slot_resolver = SlotResolver(self)
    ...
    def _register_skill_handlers(self) -> None:
        ...
        for name, handler in {
            "read_applicant": self._read_applicant_node,
            "read_workspace": self._read_workspace_node,
            "read_workflow_catalog": self._read_workflow_catalog_node,
            "read_workflow_schema": self._read_workflow_schema_node,
            "read_workflow_source": self._read_workflow_source_node,
            "read_attachment": self._read_attachment_node,
            "read_approver": self._read_approver_node,
            "read_project": self._read_project_node,
            "read_expense_category": self._read_expense_category_node,
            "read_expense_subclass": self._read_expense_subclass_node,
            "read_oa": self._read_oa_node,
            "read_booking": self._read_booking_node,
            "read_rooms": self._read_rooms_node,
            "read_schedule": self._read_schedule_node,
            "read_occupancy": self._read_occupancy_node,
            "read_participant_people": self._read_participant_people_node,
            "read_participants": self._read_participants_node,
        }.items():
            self.node_args_handlers.register(name, handler)
    ...
    def _default_skill_tool_args(
        ...
        action = self._skill_node_tool_action(state, runtime, node)
        if action is None:
            return NodeDirective("waiting")
        return NodeDirective("ready", actions=[action])

    @staticmethod
    def _read_actions_directive(actions: list[StepAction]) -> NodeDirective:
        return NodeDirective("ready", actions=actions) if actions else NodeDirective("waiting")

    @staticmethod
    def _read_action(tool: str, args: dict[str, Any], **options: Any) -> StepAction:
        action = StepAction("tool", tool, args)
        action.read_options = dict(options)
        return action

    def _actions_from_read_tasks(self, tasks: list[ReadTask]) -> list[StepAction]:
        ...
        for task in tasks:
            actions.append(
                self._read_action(
                    ...
                    mapping_score=task.mapping_score,
                    parallel_eligible=task.parallel_eligible,
                )
            )
        return actions

    def _read_applicant_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        return NodeDirective("ready", actions=[self._read_action("user.get_info", {})])

    def _read_workspace_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        if str(node.get("validator") or "") == "meeting.workspace_optional" and not runtime.slots.get("needs_workspace"):
            return NodeDirective("passed", evidence_refs=["task.slots:no_workspace_lookup_required"])
        return NodeDirective("ready", actions=[self._read_action("user.get_workspace", {})])

    def _read_workflow_catalog_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        kind = "leave" if "leave" in runtime.capability else "expense_material"
        return NodeDirective(
            "ready",
            actions=[self._read_action("workflow.catalog", {"keyword": self.workflow_registry.catalog_keyword(kind)})],
        )

    def _read_workflow_schema_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        kind = "leave" if "leave" in runtime.capability else "expense_material"
        return NodeDirective(
            "ready",
            actions=[self._read_action("workflow.schema", {"workflow_id": self._workflow_id(kind)})],
        )

    def _read_workflow_source_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        return NodeDirective(
            "ready",
            actions=[self._read_action("oa.done.list", {"keyword": self.workflow_registry.oa_keyword("leave")})],
        )

    def _read_attachment_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        directory = self._leave_attachment_directory(state)
        if not directory:
            return NodeDirective("passed", evidence_refs=["user_text:no_attachment_required"])
        return NodeDirective("ready", actions=[self._read_action("file.list", {"directory": directory})])

    def _read_approver_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        plan = state.workflow.evidence.get("leave_plan") if isinstance(state.workflow.evidence.get("leave_plan"), dict) else {}
        args = self._next_approver_search_args(state, plan, mark_attempt=False) if plan else None
        return self._read_actions_directive([self._read_action("workflow.search_person", args)] if args else [])

    def _read_project_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        evidence = state.workflow.evidence
        if evidence.get("verified_project"):
            return NodeDirective("passed", evidence_refs=["workflow.project_search"])
        projects = evidence.get("project_candidates") if isinstance(evidence.get("project_candidates"), list) else []
        if projects:
            selected = self._select_project_candidate_with_llm(state, projects)
            if selected:
                evidence["verified_project"] = selected
                self._bind_expense_project(state, selected, "skill_project_candidate_selection")
                return NodeDirective("passed", evidence_refs=["workflow.project_search"])
            return NodeDirective("failed", reason="ambiguous_project")
        ...
        return self._read_actions_directive(self._actions_from_read_tasks(tasks))

    def _read_expense_category_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        if state.workflow.evidence.get("category_options"):
            return NodeDirective("passed", evidence_refs=["workflow.browser_search:category"])
        field_id = self._expense_category_field_id(state)
        if not field_id:
            return NodeDirective("failed", reason="workflow_schema_lookup_contract_missing")
        return NodeDirective(
            "ready",
            actions=[
                self._read_action(
    ...
    def _read_expense_subclass_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        tasks: list[ReadTask] = []
        self._append_expense_subclass_read_task(state, tasks)
        return self._read_actions_directive(self._actions_from_read_tasks(tasks))

    def _read_oa_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        tool = str(node.get("tool") or "")
        if tool not in {"oa.todo.list", "oa.done.list"}:
            return NodeDirective("failed", reason="oa_lookup_error")
        workflow_kind = "leave" if "leave" in runtime.capability else "expense_material"
        return NodeDirective(
            "ready",
            actions=[self._read_action(tool, {"keyword": self.workflow_registry.oa_keyword(workflow_kind)})],
        )

    def _read_booking_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        mr = state.meetingroom
        node_id = str(node.get("id") or "")
        if node_id == "query_booking":
            args: dict[str, Any] = {}
            day = runtime.slots.get("day") or self._meeting_day(state)
            if day:
                args["day"] = day
            if runtime.slots.get("keyword"):
                args["keyword"] = runtime.slots["keyword"]
        elif runtime.capability.startswith("meeting.participant_"):
            args = self._next_participant_booking_list_args(state) or {}
        else:
            args = self._next_booking_list_args(state) or {}
        if node_id == "locate" and (runtime.slots.get("order_id") or mr.evidence.get("selected_booking")):
            if runtime.slots.get("order_id") and not mr.evidence.get("selected_booking"):
                mr.evidence["selected_booking"] = {
                    key: value
                    for key, value in {
                        "order_id": runtime.slots.get("order_id"),
                        "day": runtime.slots.get("day"),
                        "start": runtime.slots.get("start"),
                        "end": runtime.slots.get("end"),
                        "title": runtime.slots.get("title") or runtime.slots.get("keyword"),
                    }.items()
                    if value not in (None, "")
                }
            return NodeDirective("passed", evidence_refs=["user_text:order_id"])
        return NodeDirective("ready", actions=[self._read_action("meetingroom.booking.list", args)])

    def _read_rooms_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        tasks: list[ReadTask] = []
        self._append_room_list_read_tasks(state, tasks)
        return self._read_actions_directive(self._actions_from_read_tasks(tasks))

    def _read_schedule_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        tasks: list[ReadTask] = []
        self._append_schedule_read_tasks(state, tasks)
        return self._read_actions_directive(self._actions_from_read_tasks(tasks))

    def _read_occupancy_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        if not self._requires_room_occupancy_read(state):
            return NodeDirective("passed", evidence_refs=["selected_booking:no_extra_occupancy_read"])
        selected = state.meetingroom.evidence.get("selected_booking") or {}
        day = str(selected.get("day") or self._meeting_day(state) or "")
        room_id = str(selected.get("room_id") or "")
        if not day or not room_id:
            return NodeDirective("failed", reason="existing_booking_not_found")
        return NodeDirective(
            "ready",
            actions=[self._read_action("meetingroom.room.bookings", {"day": day, "room_id": room_id})],
        )

    def _read_participant_people_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        people = runtime.slots.get("participants") if isinstance(runtime.slots.get("participants"), list) else []
        actions: list[StepAction] = []
        for index, person in enumerate(people):
            if not isinstance(person, dict) or person.get("user_id") or person.get("employee_no"):
                continue
            if state.meetingroom.evidence.get(f"participant_user_{index}"):
                continue
            ...
            if keyword:
                actions.append(
                    self._read_action(
                        "user.get_info",
                        {"keyword": keyword},
                        owner_metadata={"participant_index": index},
                    )
                )
        if not actions:
            validation = self._validate_meetingroom_skill_node(state, node)
            return NodeDirective(
                str(validation.get("status") or "waiting"),
                reason=str(validation.get("reason") or ""),
                evidence_refs=list(validation.get("evidence_refs") or []),
            )
        return NodeDirective("ready", actions=actions)

    def _read_participants_node(self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]) -> NodeDirective:
        if str(node.get("validator") or "") == "meeting.participants_optional" and (
            runtime.capability != "meeting.participant_add" or not self._participant_duplicate_check_required(state)
        ):
            return NodeDirective("passed", evidence_refs=["task.slots:no_duplicate_check_required"])
        selected = state.meetingroom.evidence.get("selected_booking") or {}
        order_id = runtime.slots.get("order_id") or selected.get("order_id")
        if not order_id:
            return NodeDirective("failed", reason="existing_booking_not_found")
        return NodeDirective(
            "ready",
            actions=[self._read_action("meetingroom.booking.participant.list", {"order_id": order_id})],
        )
    ...
    def _default_skill_compute(
        ...
        node_id = str(node.get("id") or "")
        cache_key = self._json_hash(
            {
                "task_id": runtime.task_id,
                "handler": str(node.get("decision_handler") or "default_compute"),
                "node_id": node_id,
                "slots": runtime.slots,
                "evidence": self._skill_compute_evidence(state, runtime, node_id),
            }
        )
        cached = state.skill_decision_cache.get(cache_key)
        if isinstance(cached, dict):
            return NodeDirective(
                str(cached.get("status") or "waiting"),
                reason=str(cached.get("reason") or ""),
                evidence_refs=list(cached.get("evidence_refs") or []),
            )
        decision = self._compute_skill_node(state, runtime, node)
        result = self._validate_registered_skill_node(state, runtime, node)
        if decision and result.get("status") == "pending":
            result = self._skill_validation("failed", decision)
        state.skill_decision_cache[cache_key] = dict(result)
        return NodeDirective(
    ...
    def _collect_input_skill_node(
        ...
        self._activate_task_runtime_view(state, runtime)
        self.slot_resolver.resolve(state, runtime)
    ...
    def _missing_task_slots(self, state: RuntimeState, runtime: TaskRuntime) -> list[str]:
        ...
        if capability == "meeting.book":
            ...
            # Business enums are resolved by leave_form from the runtime schema.
            # collect_input only blocks facts that cannot be derived or looked up.
            for key in ("day_text", "start", "end"):
                if key == "day_text" and (leave.get("day_text") or leave.get("day")):
                    continue
                if not leave.get(key):
                    missing.append(key)
            if not (
                leave.get("approver_keyword") or leave.get("approver_title") or leave.get("approver_employee_no")
                or leave.get("approver_name_hint") or leave.get("approver_title_hint") or leave.get("approver_raw")
            ):
            ...
            if state.obs.get("mode") == "multi_turn":
                missing_name = self._expense_missing_slot_name(state)
                if missing_name:
                ...
                if not expense.get("total_amount") and not expense.get("items"):
    ...
    def _skill_node_tool_action(
        ...
        selected_room = mr.evidence.get("pending_selected_room") if isinstance(mr.evidence.get("pending_selected_room"), dict) else {}

        if tool == "meetingroom.booking.create" and selected_room:
            next_segment = self._next_segment_to_create(mr)
            if next_segment:
                if next_segment.get("day"):
                    mr.slots["day"] = next_segment["day"]
                mr.slots["start"] = next_segment.get("start")
                mr.slots["end"] = next_segment.get("end")
                mr.slots["title"] = next_segment.get("title")
            action = StepAction("tool", tool, self._booking_create_args(state, selected_room))
        elif tool == "meetingroom.booking.cancel" and (selected_booking.get("order_id") or runtime.slots.get("order_id")):
            if runtime.capability in {"meeting.cancel_rebook", "meeting.rebook_larger"}:
                if not selected_room.get("room_id"):
                    return StepAction("block_meetingroom", args={"reason": "replacement_room_not_verified"})
                if state.step_budget - state.steps_used < 2:
                    return StepAction("block_meetingroom", args={"reason": "insufficient_step_budget"})
            action = StepAction(
                "tool",
                tool,
                {"order_id": selected_booking.get("order_id") or runtime.slots.get("order_id")},
            )
        elif tool == "meetingroom.booking.extend" and (selected_booking.get("order_id") or runtime.slots.get("order_id")):
            action = StepAction(
                "tool",
                tool,
                {
                    "order_id": selected_booking.get("order_id") or runtime.slots.get("order_id"),
                    "minutes": mr.slots.get("duration_minutes") or 30,
                },
            )
        elif tool in {"meetingroom.booking.participant.add", "meetingroom.booking.participant.remove"}:
            ...
            person = participants[index] if index < len(participants) and isinstance(participants[index], dict) else {}
            users = (mr.evidence.get(f"participant_user_{index}") or {}).get("users") or []
            user_id = person.get("user_id") or (users[0].get("user_id") if users else None)
            order_id = mr.slots.get("order_id") or selected_booking.get("order_id")
            if order_id and user_id:
                participant_tool = (
                    "meetingroom.booking.participant.remove"
                    if runtime.capability == "meeting.participant_remove"
                    else "meetingroom.booking.participant.add"
                )
                action = StepAction("tool", participant_tool, {"order_id": order_id, "user_id": user_id})
        elif tool == "workflow.delete":
            source = self._replacement_leave_source_item(state)
            if source and source.get("request_id"):
                action = StepAction("tool", tool, {"request_id": source["request_id"]})
        elif tool == "workflow.save":
            draft = wf.evidence.get("workflow_skill_draft_ir") if isinstance(wf.evidence.get("workflow_skill_draft_ir"), dict) else {}
            save_args = draft.get("save_args") if isinstance(draft.get("save_args"), dict) else {}
            if runtime.intent == "leave" and wf.evidence.get("save_done"):
                next_plan = self._next_recurring_leave_plan(state)
                approver = wf.evidence.get("selected_approver") if isinstance(wf.evidence.get("selected_approver"), dict) else {}
                if next_plan and approver:
                    save_args = self._leave_save_args(state, next_plan, approver)
            ...
            if save_args:
                action = StepAction("tool", tool, save_args)
        elif tool in {"oa.todo.list", "oa.done.list"}:
            workflow_kind = "leave" if "leave" in runtime.capability else "expense_material"
            action = StepAction("tool", tool, {"keyword": self.workflow_registry.oa_keyword(workflow_kind)})
    ...
    def _skill_compute_evidence(
        self, state: RuntimeState, runtime: TaskRuntime, node_id: str
    ) -> dict[str, Any]:
        evidence = state.meetingroom.evidence if runtime.domain == "meetingroom" else state.workflow.evidence
        keys_by_node = {
            "select_room": ["room_candidates", "schedules", "selected_booking"],
            "leave_form": ["schema"],
            "approver_resolved": ["approver_searches", "approver_search_attempts"],
            "select_category": ["category_options", "verified_project", "expense_bindings"],
            "draft_ir": [
                "applicant", "schema", "verified_project", "selected_material_category",
                "subclass_options", "expense_bindings", "selected_approver", "leave_plan",
            ],
        }
        return {key: evidence.get(key) for key in keys_by_node.get(node_id, []) if key in evidence}

    def _compute_skill_node(
        self, state: RuntimeState, runtime: TaskRuntime, node: dict[str, Any]
    ) -> str:
        node_id = str(node.get("id") or "")
        if runtime.domain == "meetingroom" and node_id == "select_room":
            ...
            if selected:
                ...
                return ""
            return "no_bookable_room"
        if runtime.domain != "workflow":
            return ""

        wf = state.workflow
        if node_id == "leave_form":
            plan = self._leave_plan(state)
            if not plan:
                return "invalid_leave_form"
            wf.evidence["leave_plan"] = dict(plan)
            wf.evidence["leave_plans"] = self._leave_plans(state, plan)
            return ""
        if node_id == "approver_resolved":
            people = self._deterministic_leave_people(state, self._collected_approver_people(wf))
            if len(people) > 1:
                people = self._select_leave_people(state, people)
            if len(people) != 1 or not people[0].get("user_id"):
                return "ambiguous_approver"
            wf.evidence["selected_approver"] = dict(people[0])
            return ""
        if node_id == "select_category":
            category = self._select_material_category(state)
            if not category:
                return "ambiguous_material_category"
            self._bind_expense_category(state, category, "skill_compute")
            return ""
        if node_id == "draft_ir" and runtime.intent == "leave":
            plan = wf.evidence.get("leave_plan") if isinstance(wf.evidence.get("leave_plan"), dict) else {}
            approver = wf.evidence.get("selected_approver") if isinstance(wf.evidence.get("selected_approver"), dict) else {}
            if not plan or not approver:
                return "invalid_leave_form" if not plan else "ambiguous_approver"
            save_args = self._leave_save_args(state, plan, approver)
            wf.evidence["workflow_skill_draft_ir"] = {
                "workflow_id": self._workflow_id("leave"),
                "submit": save_args.get("submit"),
                "save_args": save_args,
            ...
            return ""
        if node_id == "draft_ir" and runtime.intent == "expense_material":
            project = wf.evidence.get("verified_project") if isinstance(wf.evidence.get("verified_project"), dict) else {}
            category = wf.evidence.get("selected_material_category") if isinstance(wf.evidence.get("selected_material_category"), dict) else {}
            if not project or not category:
                return "missing_required_info"
            save_args_or_reason = self._expense_ir_save_args_or_block(state, project, category)
            if isinstance(save_args_or_reason, str):
                return save_args_or_reason
            wf.evidence["workflow_skill_draft_ir"] = {
    ...
    def run(self, case_id: str) -> dict:
        ...
        try:
            ...
            self.tool_registry = self.tool_contract_reconciler.reconcile(tools)
            self.tool_adapter.set_tool_registry(self.tool_registry)
            self.preflight_guard.tool_registry = self.tool_registry
            state = RuntimeState(
                obs=obs,
                tools=set(self.tool_registry.available_tools),
            ...
            self._debug_log(
                debug_config,
                {
                    ...
                    "obs": obs,
                    "network": {
                        "fast": self._network_diagnostics(self._llm_config("fast")),
                        "strong": self._network_diagnostics(self._llm_config("strong")),
                    },
                },
            )
            ...
            self._debug_log(
                debug_config,
                {
                    ...
                    "tool_reconciliation": self.tool_registry.reconciliation,
            ...
            total_skill_nodes = sum(
                len(runtime.skill_run.nodes) for runtime in state.task_runtimes if runtime.skill_run is not None
            )
            max_iterations = max(8, state.step_budget + total_skill_nodes * 2)
            for _ in range(max_iterations):
                ...
                if self.skill_scheduler.drain_reads(state, debug_config):
                    ...
                    if self._all_done(state):
                        break
                    continue
                decision = self.skill_scheduler.next_action(state, debug_config)
                if decision.action is None:
                    self._advance_task_runtimes(state)
                    if decision.progressed:
                        continue
                    if self.skill_scheduler.settle_exhausted_postchecks(state, debug_config):
                        self._refresh_task_runtime_statuses(state)
                        self._advance_task_runtimes(state)
                        continue
                    if self._all_done(state):
                        break
                    self._block_scheduler_stall(state, decision.diagnostic, debug_config)
                    break
                if decision.action.kind in {"tool", "reply"} and state.steps_used >= state.step_budget:
                    diagnostic = self.skill_scheduler.stall_diagnostic(state)
                    diagnostic["reason"] = "insufficient_step_budget"
                    diagnostic["blocked_action"] = {
                        "kind": decision.action.kind,
                        "tool": decision.action.tool,
                    }
                    self._block_scheduler_stall(state, diagnostic, debug_config)
                    break
                self._execute(
                    state,
                    decision.action,
                    debug_config,
                    decision.action.execution_context,
                )
                self._advance_task_runtimes(state)
                if self._all_done(state):
                    break

            if not self._all_done(state) and not state.scheduler_stalls:
                self._block_scheduler_stall(
                    state,
                    self.skill_scheduler.stall_diagnostic(state),
                    debug_config,
                )
            ...
            self._debug_log(
                debug_config,
                {
                    ...
                    "llm_elapsed": round(llm_elapsed_seconds, 3),
                    "llm_elapsed_seconds": round(llm_elapsed_seconds, 3),
                    "semantic_elapsed": round(state.semantic_elapsed_seconds, 3),
                    "semantic_elapsed_seconds": round(state.semantic_elapsed_seconds, 3),
                    ...
                    "tool_elapsed": round(state.tool_elapsed_seconds, 3),
                    ...
                    "read_elapsed": round(state.read_elapsed_seconds, 3),
                    ...
                    "read_tasks_failed": state.read_tasks_failed,
                    "read_tasks_retried": state.read_tasks_retried,
                    "read_retry_exhausted": state.read_retry_exhausted,
                    ...
                    "non_llm_elapsed": round(max(0.0, elapsed_seconds - llm_elapsed_seconds), 3),
    ...
    def _task_graph_max_attempts(self) -> int:
        return 1
    ...
    def _llm_config(self, profile: str = "strong") -> dict[str, Any]:
        ...
        llm["max_calls"] = 1 if profile == "fast" else 2
        try:
    ...
    def _network_diagnostics(self, llm_config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "endpoint": endpoint(llm_config.get("base_url")),
            ...
            "detected_proxies": {
                str(name): endpoint(value)
                for name, value in sorted(proxies.items())
                if name.lower() in {"http", "https", "all"}
            },
    ...
    def _extract_semantics(self, state: RuntimeState, obs: dict[str, Any], tools: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def finish(semantic: dict[str, Any], reason: str = "") -> dict[str, Any]:
            ...
            state.semantic_elapsed_seconds = max(0.0, time.monotonic() - stage_started)
        ...
        if not self._can_call_llm(state, "fast"):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            attempt_config["max_calls"] = max_attempts
            attempt_config["transport_max_attempts"] = 1
            attempt_config["seed"] = None
            fast_attempted = True
            state.semantic_attempts = attempt
            ...
            try:
                content = self._chat_completion(
                    ...
                    call_purpose="semantic_task_graph",
                )
                response_chars = len(content)
                parsed = self._parse_json_object(content)
                if not isinstance(parsed, dict):
                    raise ValueError("invalid_json")
                parsed = self._expand_sparse_semantic_result(parsed, full_query)
                merged = self._merge_semantic(baseline, parsed)
                source = "fast_llm"
                fast_llm_success = True
                self._log_semantic_attempt(state, attempt_config, attempt, attempt_started, True, "")
                return finish(merged)
            except Exception as exc:
                fallback_reason = str(exc)[:200] or "fast_llm_error"
                retryable = self._semantic_error_retryable(exc)
                self._log_semantic_attempt(
                    state, attempt_config, attempt, attempt_started, False, fallback_reason, retryable
                )
    ...
    def _semantic_error_retryable(self, exc: Exception) -> bool:
        ...
        if any(marker in text for marker in ["401", "403", "400", "authentication", "unauthorized", "invalid api key"]):
            return False
        if isinstance(exc, (TimeoutError, urllib.error.URLError, json.JSONDecodeError, ValueError)):
            return True
        return any(
            marker in text
            for marker in ["timeout", "timed out", "empty_llm_response", "invalid_json", "http 500", "http 502", "http 503", "http 504"]
        )
    ...
    def _log_semantic_attempt(
        ...
        self._debug_log(
            llm_config,
            {
                ...
                "success": success,
                "retryable": retryable,
                "timeout_seconds": round(float(llm_config.get("timeout") or 0), 3),
    ...
    @staticmethod
    def _llm_request_timeout(requested: int | float) -> float:
        _ = requested
        return 30.0
    ...
    def _chat_completion(
        ...
        call_limit = int(llm_config.get("max_calls") or 1)
        if state is not None and not self._can_call_llm(state, call_profile, max_calls=call_limit):
            raise RuntimeError(f"{call_profile} llm call budget exhausted")
        self._note_llm_call(state, call_profile)
        call_started = time.monotonic()
        if state is not None:
            state.llm_call_sequence += 1
            call_id = f"{state.obs.get('case_id') or 'case'}:{call_profile}:{state.llm_call_sequence}"
        else:
            call_id = f"no-state:{call_profile}:{time.time_ns()}"
        call_purpose = str(call_purpose or context_pack_type or "unspecified")
        messages = [dict(item) for item in messages if isinstance(item, dict)]
        user_message_indexes = [index for index, item in enumerate(messages) if item.get("role") == "user"]
        if user_message_indexes and not any(
            "json" in str(messages[index].get("content") or "").lower() for index in user_message_indexes
        ):
            index = user_message_indexes[-1]
            messages[index]["content"] = str(messages[index].get("content") or "") + "\nReturn response as json."
        elif not user_message_indexes:
            messages.append({"role": "user", "content": "Return response as json."})
        if not prompt_chars:
        ...
        configured_max_tokens = int(llm_config.get("max_tokens") or 0)
        requested_max_tokens = int(max_output_tokens or 0)
        if configured_max_tokens > 0 and requested_max_tokens > 0:
            body["max_tokens"] = min(configured_max_tokens, requested_max_tokens)
        elif configured_max_tokens > 0 or requested_max_tokens > 0:
            body["max_tokens"] = max(configured_max_tokens, requested_max_tokens)
        ...
        def send_request(request_body: dict[str, Any], request_timeout: float) -> dict[str, Any]:
            nonlocal attempt_count, response_status
            attempt_count += 1
        ...
        def send_once(request_body: dict[str, Any], request_timeout: float) -> dict[str, Any]:
            value = send_request(request_body, request_timeout)
            choices = value.get("choices") if isinstance(value, dict) else []
            content = ((choices or [{}])[0].get("message") or {}).get("content") if choices else ""
        ...
        try:
            configured_timeout = float(llm_config.get("timeout") or 30)
            timeout = self._llm_request_timeout(configured_timeout)
            if state is not None:
                self._debug_log(
                    llm_config,
                    {
                        ...
                        "prompt_hash": self._json_hash(body.get("messages") or []),
                        "network": self._network_diagnostics(llm_config),
                    },
                )
            try:
                payload = send_once(body, timeout)
                ...
                if not seed_rejected:
                    ...
                    raise
                # A rejected optional seed is still one failed logical HTTP call.
                # The next model decision can omit seed, but this call is not retried.
                seed_fallback = True
                setattr(exc, "_agent_error_text", error_text)
                raise
        except urllib.error.HTTPError as exc:
            ...
            self._log_llm_call_event(
                ...
                response_status=int(getattr(exc, "code", 0) or response_status),
            )
    ...
    def _log_llm_call_event(
        ...
        self._debug_log(
            llm_config,
            {
                ...
                "case_id": state.obs.get("case_id"),
                "profile": profile,
                "purpose": call_purpose or context_pack_type,
                "model": llm_config.get("model"),
                ...
                "success": success,
                "error_class": self._llm_error_class(error),
                "attempt_count": int(attempt_count or 0),
                "seed_fallback": bool(seed_fallback),
                ...
                "prompt_chars": int(prompt_chars or 0),
                "response_chars": response_chars,
                "request_bytes": len(json.dumps(request_body or {}, ensure_ascii=False).encode("utf-8")),
                "response_bytes": len(json.dumps(response_payload or {}, ensure_ascii=False).encode("utf-8")),
                "response_json_valid": bool(response_content and self._parse_json_object(response_content) is not None),
                "usage": (response_payload or {}).get("usage") if isinstance((response_payload or {}).get("usage"), dict) else {},
                "network": self._network_diagnostics(llm_config),
    ...
    def _llm_error_class(self, error: Any) -> str:
        ...
        if "http" in text:
            return "http_error"
        if any(word in text for word in ["connection", "network", "dns", "name or service", "reset by peer"]):
            return "network_error"
        if any(word in text for word in ["no choices", "empty content", "invalid json"]):
    ...
    def _heuristic_meetingroom(self, query: str, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        meeting_actions = ["取消", "延长", "重订", "重新预订", "换大", "加入", "加到", "移除", "哪些人", "有哪些人", "参加"]
        room_ids = self._extract_room_ids(query)
        has_explicit_meeting_signal = bool(room_ids) or any(word in query for word in explicit_meeting_terms) or any(
        ...
        booking_directive = bool(re.search(r"(?:选[^，。；;]{0,12})?(?:订|预订)(?!情况|记录)", query))
        intent = "book_single"
        ...
        elif booking_directive and len(room_ids) >= 2 and any(word in query for word in ["更空闲", "最空闲", "哪个空闲", "哪个更空", "比较"]):
            intent = "book_by_schedule_analysis"
        elif "日程" in query and booking_directive:
            intent = "book_by_schedule_analysis"
        ...
        if not office_candidates and "小镇" in query and re.search(r"[一二三四五六七八九十\d]\s*(?:楼|层|F)", query, flags=re.I):
    ...
    def _heuristic_workflow(self, query: str, obs: dict[str, Any]) -> dict[str, Any]:
        has_leave = ("请" in query and "假" in query) or any(
            word in query for word in ["事假", "年假", "病假", "育儿假", "年休假", "婚假", "陪产假", "丧假"]
        )
        has_expense = self._has_expense_signal(query)
        if not has_leave and not has_expense:
            return {"intent": "unknown"}
        submit = self._default_leave_submit_intent(query) if has_leave and not has_expense else self._submit_intent(query)
        if has_expense and not has_leave:
            ...
            leave_index = min(
                [
                    i
                    for i in [
                        query.find("请假"), query.find("事假"), query.find("年假"), query.find("年休假"),
                        query.find("病假"), query.find("育儿假"), query.find("婚假"), query.find("陪产假"), query.find("丧假"),
                    ]
                    if i >= 0
                ]
                or [9999]
    ...
    def _expand_sparse_semantic_result(self, parsed: dict[str, Any], query: str) -> dict[str, Any]:
        """Expand the compact LLM contract into the existing internal semantic shape."""
        if isinstance(parsed.get("task_graph"), dict) or not isinstance(parsed.get("tasks"), list):
            return parsed
        tasks: list[dict[str, Any]] = []
        domains: list[str] = []
        meeting: dict[str, Any] = {"intent": "unknown"}
        workflow: dict[str, Any] = {"intent": "unknown"}
        for index, raw in enumerate(parsed.get("tasks") or []):
            ...
            if domain not in domains:
                domains.append(domain)
            if domain == "meetingroom" and meeting.get("intent") == "unknown":
                meeting = {"intent": intent, **slots}
            elif domain == "workflow" and workflow.get("intent") == "unknown":
                workflow = {"intent": intent, "submit": submit}
                if intent == "leave":
                    workflow["leave"] = slots
                elif intent == "expense_material":
                    workflow["expense"] = slots
        return {
            "domains": domains,
    ...
    def _domain_source_text(self, query: str, domain: str, intent: str | None = None) -> str:
        ...
        if domain == "meetingroom":
            ...
            if not positions:
                return text
            trigger = min(positions)
            hard_start = max(text.rfind(marker, 0, trigger) for marker in ["。", "；", ";", "\n"])
            connector_words = ["另一个任务", "顺便", "另外", "同时", "并且", "然后", "还有", "再帮"]
            connector_positions = [(text.rfind(word, 0, trigger), word) for word in connector_words]
            connector_pos, connector_word = max(connector_positions, key=lambda item: item[0])
            if connector_pos > hard_start:
                start = connector_pos + len(connector_word)
            else:
                start = hard_start + 1
            workflow_positions = [
    ...
    def _stabilize_workflow_semantic(self, baseline: dict[str, Any], merged: dict[str, Any]) -> None:
        ...
        # Draft/submit is a policy decision, not a free-form model slot.
        if "submit" in baseline_wf:
            merged_wf["submit"] = bool(baseline_wf.get("submit"))
        baseline_leave = baseline_wf.get("leave") if isinstance(baseline_wf.get("leave"), dict) else {}
        merged_leave = merged_wf.setdefault("leave", {}) if isinstance(merged_wf, dict) else {}
        if isinstance(merged_leave, dict):
            for key, value in baseline_leave.items():
                if value not in (None, "", [], {}):
                    merged_leave[key] = value
    ...
    def _sync_workflow_skill(self, state: RuntimeState) -> None:
        ...
        for node in skill.nodes:
            ...
            if skill.statuses.get(node_id) in SkillRun.TERMINAL_STATUSES:
                continue
            if skill.statuses.get(node_id) != "running":
                continue
    ...
    def _skill_validation(
        ...
        fingerprint = ""
        if status == "failed":
            fingerprint = self._json_hash({"reason": reason, "value": fingerprint_value})
        return {
            "status": status,
            "reason": reason,
    ...
    def _validate_workflow_skill_node(self, state: RuntimeState, node: dict[str, Any]) -> dict[str, Any]:
        validator = str(node.get("validator") or "")
        wf = state.workflow
        evidence = wf.evidence
        if validator == "workflow.applicant_verified":
            applicant = evidence.get("applicant") if isinstance(evidence.get("applicant"), dict) else {}
            if not applicant:
                return self._skill_validation("pending")
            if applicant.get("user_id") and applicant.get("employee_no"):
                return self._skill_validation("passed", evidence_refs=["user.get_info"])
            return self._skill_validation("failed", "invalid_applicant", fingerprint_value=applicant)
        if validator == "workflow.catalog_verified":
            catalog = evidence.get("catalog") if isinstance(evidence.get("catalog"), dict) else {}
            if not catalog:
                return self._skill_validation("pending")
            capability = self._active_task_capability(state, "workflow")
            expected = self._workflow_id("leave" if "leave" in capability else "expense_material")
            ...
            return self._skill_validation("failed", "workflow_not_found", fingerprint_value=catalog)
        if validator == "workflow.schema_verified":
            schema_result = evidence.get("schema") if isinstance(evidence.get("schema"), dict) else {}
            if not schema_result:
                return self._skill_validation("pending")
            schema = schema_result.get("schema") if isinstance(schema_result.get("schema"), dict) else schema_result
            if isinstance(schema, dict) and schema.get("required_fields"):
                return self._skill_validation("passed", evidence_refs=["workflow.schema"])
            return self._skill_validation("failed", "invalid_workflow_schema", fingerprint_value=schema_result)
        if validator == "workflow.leave_source_verified":
            if "replacement_source_lookup" not in evidence:
                return self._skill_validation("pending")
            source = self._replacement_leave_source_item(state)
            if source:
                return self._skill_validation("passed", evidence_refs=["oa.done.list"])
            return self._skill_validation("failed", "existing_workflow_not_found", fingerprint_value=evidence.get("replacement_source_lookup"))
        if validator == "workflow.leave_source_deleted":
            deleted = evidence.get("replacement_delete_done") if isinstance(evidence.get("replacement_delete_done"), dict) else {}
            if deleted:
                return self._skill_validation("passed", evidence_refs=["workflow.delete"])
            return self._skill_validation("pending")
        if validator == "workflow.leave_form_valid":
            plan = evidence.get("leave_plan") if isinstance(evidence.get("leave_plan"), dict) else {}
            if not plan:
                return self._skill_validation("pending")
            required = ["start_time", "end_time", "leave_type", "reason", "duration"]
            if all(plan.get(key) not in (None, "") for key in required):
                return self._skill_validation("passed", evidence_refs=["user_text"])
            return self._skill_validation("failed", "invalid_leave_form", fingerprint_value=plan)
        if validator == "workflow.leave_attachment_verified":
            directory = self._leave_attachment_directory(state)
            if not directory:
                return self._skill_validation("passed", evidence_refs=["user_text:no_attachment_required"])
            if not evidence.get("file_list_attempted"):
                return self._skill_validation("pending")
            files = (evidence.get("file_list") or {}).get("files") or []
            if files:
                return self._skill_validation("passed", evidence_refs=["file.list"])
            return self._skill_validation("failed", "leave_attachment_not_found", fingerprint_value=evidence.get("file_list_attempted"))
        if validator == "workflow.approver_search_verified":
            attempts = evidence.get("approver_search_attempts") or []
            if not attempts:
                return self._skill_validation("pending")
            if self._collected_approver_people(wf):
                return self._skill_validation("passed", evidence_refs=["workflow.search_person"])
            return self._skill_validation("failed", "empty_approver_result", fingerprint_value=len(attempts))
        if validator == "workflow.approver_resolved":
            selected = evidence.get("selected_approver") if isinstance(evidence.get("selected_approver"), dict) else {}
            if selected.get("user_id"):
                return self._skill_validation("passed", evidence_refs=["workflow.search_person"])
            return self._skill_validation("pending")
        if validator == "workflow.expense_project_verified":
            project = evidence.get("verified_project") if isinstance(evidence.get("verified_project"), dict) else {}
            if not project:
                return self._skill_validation("pending")
            if project.get("project_code") and project.get("wbs_code") and evidence.get("project_search_history"):
                return self._skill_validation("passed", evidence_refs=["workflow.project_search"])
            return self._skill_validation("failed", "invalid_project_evidence", fingerprint_value=project)
        if validator == "workflow.expense_category_verified":
            options = (evidence.get("category_options") or {}).get("options") or []
            selected = evidence.get("selected_material_category") if isinstance(evidence.get("selected_material_category"), dict) else {}
            if not options or not selected:
                return self._skill_validation("pending")
            allowed = {str(item.get("value") or item.get("code") or "") for item in options if isinstance(item, dict)}
            selected_id = self._expense_category_id(selected)
            if selected_id and selected_id in allowed:
                return self._skill_validation("passed", evidence_refs=["workflow.browser_search:category"])
            return self._skill_validation("failed", "invalid_material_category", fingerprint_value={"selected": selected_id, "allowed": sorted(allowed)})
        if validator == "workflow.expense_category_candidates":
            options = (evidence.get("category_options") or {}).get("options") or []
            return (
                self._skill_validation("passed", evidence_refs=["workflow.browser_search:category"])
                if options
                else self._skill_validation("pending")
            )
        if validator == "workflow.expense_subclass_verified":
            options = (evidence.get("subclass_options") or {}).get("options") or []
            if not options:
                return self._skill_validation("pending")
            bindings = evidence.get("expense_bindings") if isinstance(evidence.get("expense_bindings"), dict) else {}
            project = evidence.get("verified_project") if isinstance(evidence.get("verified_project"), dict) else {}
            category = evidence.get("selected_material_category") if isinstance(evidence.get("selected_material_category"), dict) else {}
            expected = f"{self._expense_project_fingerprint(project)}|{self._expense_category_id(category)}"
            binding = bindings.get("subclass") if isinstance(bindings.get("subclass"), dict) else {}
            if binding.get("dependency_fingerprint") == expected and binding.get("allowed_ids"):
                return self._skill_validation("passed", evidence_refs=["workflow.browser_search:subclass"])
            return self._skill_validation("failed", "stale_subclass_candidates", fingerprint_value={"expected": expected, "binding": binding})
        if validator in {"workflow.leave_draft_valid", "workflow.expense_draft_valid"}:
            draft = evidence.get("workflow_skill_draft_ir") if isinstance(evidence.get("workflow_skill_draft_ir"), dict) else {}
            save_args = draft.get("save_args") if isinstance(draft.get("save_args"), dict) else {}
            if not save_args:
                return self._skill_validation("pending")
            preflight = self.preflight_guard.validate_write(state, "workflow.save", save_args)
            if preflight.get("passed"):
                return self._skill_validation("passed", evidence_refs=preflight.get("evidence_refs") or [])
            return self._skill_validation("failed", str(preflight.get("reason") or "workflow_preflight_failed"), fingerprint_value=preflight)
        if validator == "workflow.save_verified":
            saved = evidence.get("save_done") if isinstance(evidence.get("save_done"), dict) else {}
            if not saved:
                return self._skill_validation("pending")
            ...
            if result.get("draft_saved"):
                ...
                if self.workflow_registry.kind_for_id(save_args.get("workflow_id")) == "leave":
                    plans = evidence.get("leave_plans") if isinstance(evidence.get("leave_plans"), list) else []
        ...
        if validator == "workflow.oa_verified":
            if evidence.get("oa_checked") is not None:
                return self._skill_validation("passed", evidence_refs=["oa.done.list" if wf.slots.get("submit") else "oa.todo.list"])
            return self._skill_validation("pending")
        return self._skill_validation("failed", "unknown_skill_validator", fingerprint_value=validator)

    def _validate_meetingroom_skill_node(self, state: RuntimeState, node: dict[str, Any]) -> dict[str, Any]:
        validator = str(node.get("validator") or "")
        mr = state.meetingroom
        evidence = mr.evidence
        if validator == "meeting.rooms_queried":
            if "room_candidates" not in evidence and not evidence.get("room_lists_by_day"):
                return self._skill_validation("pending")
            rooms = (evidence.get("room_candidates") or {}).get("rooms") or []
            day_rooms = [
                room
                for result in (evidence.get("room_lists_by_day") or {}).values()
                if isinstance(result, dict)
                for room in result.get("rooms") or []
                if isinstance(room, dict)
            ]
            if rooms or day_rooms:
                return self._skill_validation("passed", evidence_refs=["meetingroom.room.list"])
            return self._skill_validation("failed", "no_bookable_room", fingerprint_value=len(evidence.get("tried_room_lists") or []))
        if validator == "meeting.schedules_queried":
            schedules = evidence.get("schedules") if isinstance(evidence.get("schedules"), dict) else {}
            expected_room_ids = {str(item) for item in mr.slots.get("room_ids") or [] if item}
            if schedules and (not expected_room_ids or expected_room_ids.issubset({str(item) for item in schedules})):
                return self._skill_validation("passed", evidence_refs=["meetingroom.room.schedule"])
            if evidence.get("room_candidates"):
                return self._skill_validation("passed", evidence_refs=["meetingroom.room.list"])
            return self._skill_validation("pending")
        if validator == "meeting.room_selected":
            selected = evidence.get("pending_selected_room")
            if selected:
                return self._skill_validation("passed", evidence_refs=["meetingroom.room.list"])
            if evidence.get("room_candidates"):
                return self._skill_validation("failed", "no_bookable_room", fingerprint_value=evidence.get("room_candidates"))
            return self._skill_validation("pending")
        if validator == "meeting.booking_created":
            return self._skill_validation("passed", evidence_refs=["meetingroom.booking.create"]) if evidence.get("create_done") else self._skill_validation("pending")
        if validator == "meeting.booking_located":
            if evidence.get("selected_booking"):
                return self._skill_validation("passed", evidence_refs=["meetingroom.booking.list"])
            if mr.slots.get("order_id"):
                return self._skill_validation("passed", evidence_refs=["user_text:order_id"])
            if "booking_query" in evidence:
                return self._skill_validation("failed", "existing_booking_not_found", fingerprint_value=evidence.get("booking_query"))
            return self._skill_validation("pending")
        ...
        if validator == "meeting.booking_extended":
            if evidence.get("extend_done"):
                return self._skill_validation("passed", evidence_refs=["meetingroom.booking.extend"])
        ...
        if validator == "meeting.participants_optional":
            if self._active_task_capability(state, "meetingroom") != "meeting.participant_add" or not self._participant_duplicate_check_required(state):
        ...
        if validator == "meeting.participants_resolved":
            participants = mr.slots.get("participants") if isinstance(mr.slots.get("participants"), list) else []
            if not participants:
                return self._skill_validation("failed", "missing_required_info", fingerprint_value="participants")
            unresolved = []
            for index, person in enumerate(participants):
                users = (evidence.get(f"participant_user_{index}") or {}).get("users") or []
                if not person.get("user_id") and not person.get("employee_no") and not users:
                    unresolved.append(index)
            if not unresolved:
                return self._skill_validation("passed", evidence_refs=["user.get_info"])
            return self._skill_validation("pending")
        if validator == "meeting.participants_updated":
            participants = mr.slots.get("participants") if isinstance(mr.slots.get("participants"), list) else []
            results = evidence.get("participant_results") or []
            if participants and len(results) >= len(participants):
                return self._skill_validation("passed", evidence_refs=["meetingroom.booking.participant.add/remove"])
            return self._skill_validation("pending")
        return self._skill_validation("failed", "unknown_skill_validator", fingerprint_value=validator)

    def _deterministic_leave_people(self, state: RuntimeState, people: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if keyword and keyword not in {"一个经", "一个经理", "经理", "一个", "某个", "任意", "找一个"}:
            filtered = [person for person in filtered if keyword in str(person.get("name") or "") or keyword in str(person.get("employee_no") or "")]
        if title:
            matched = [person for person in filtered if title in str(person.get("title") or "") or title in str(person.get("name") or "")]
            if matched:
                filtered = matched
        if department:
    ...
    def _mark_workflow_skill_action_running(self, state: RuntimeState, tool: str) -> None:
        ...
        for node in skill.ready_nodes():
            node_tools = [str(node.get("tool") or ""), *[str(item) for item in node.get("tools") or []]]
    ...
    def _owner_task_is_runnable(self, state: RuntimeState, task: ReadTask) -> bool:
        ...
        for item in task.owners:
            owner = by_id.get(str(item.get("task_id") or ""))
            if owner is None or owner.status in TaskRuntime.TERMINAL_STATUSES:
                continue
            if all(by_id.get(task_id) is None or by_id[task_id].status == "completed" for task_id in owner.depends_on):
                return True
        return False
    ...
    def _active_task_capability(self, state: RuntimeState, domain: str) -> str:
        ...
        return str(domain_state.intent or "")
    ...
    def _activate_task_runtime_view(self, state: RuntimeState, runtime: TaskRuntime) -> DomainState:
        ...
        if previous is not None and previous is not runtime:
            previous.local_evidence.update(
                {
    ...
    def _task_local_evidence_keys(self, domain: str, evidence: dict[str, Any]) -> set[str]:
        # Runtime evidence belongs to the task that produced it. Cached reads
        # are re-applied for another owner, so no mutable business evidence has
        # to leak through DomainState to make read de-duplication work.
        return set(evidence)

    def _capture_task_runtime_evidence(self, state: RuntimeState, runtime: TaskRuntime) -> None:
        domain_state = state.meetingroom if runtime.domain == "meetingroom" else state.workflow
        runtime.local_evidence.update(
            {
                key: domain_state.evidence[key]
                for key in self._task_local_evidence_keys(runtime.domain, domain_state.evidence)
                if key in domain_state.evidence
            }
        )
    ...
    def _task_terminal_result(self, state: RuntimeState, runtime: TaskRuntime) -> dict[str, Any]:
        evidence = runtime.local_evidence or (
            state.meetingroom.evidence if runtime.domain == "meetingroom" else state.workflow.evidence
        )
        if runtime.status == "blocked":
            save_done = evidence.get("save_done") if isinstance(evidence.get("save_done"), dict) else {}
            if runtime.domain == "workflow" and save_done:
                saved = self._workflow_result_from_save(
                    save_done.get("args") or {},
                    save_done.get("result") or {},
                )
                if saved:
                    return saved
            reason = self._precise_runtime_block_reason(state, runtime, evidence)
            result = {"status": "blocked", "reason": reason}
            if runtime.domain == "meetingroom" and runtime.capability in {
                "meeting.cancel",
                "meeting.extend",
                "meeting.cancel_rebook",
                "meeting.rebook_larger",
            }:
                selected = evidence.get("selected_booking") if isinstance(evidence.get("selected_booking"), dict) else {}
                order_id = selected.get("order_id") or runtime.slots.get("order_id")
                if order_id:
        ...
        if capability == "meeting.query_booking":
            query_result = evidence.get("booking_query") if isinstance(evidence.get("booking_query"), dict) else {}
            bookings = query_result.get("bookings") or []
            result = {
                "status": "queried",
                "day": runtime.slots.get("day") or self._meeting_day(state),
                "keyword": runtime.slots.get("keyword") or "",
                "count": len(bookings),
                "bookings": bookings,
            }
            return {key: value for key, value in result.items() if value not in (None, "")}
        if capability == "meeting.query_room_schedule":
            room_ids = list((evidence.get("schedules") or {}).keys())
            start_date, end_date = self._schedule_range(state)
            result = {
                "status": "queried",
                "room_ids": room_ids,
                "room_id": room_ids[0] if len(room_ids) == 1 else "",
    ...
    def _precise_runtime_block_reason(
        self,
        state: RuntimeState,
        runtime: TaskRuntime,
        evidence: dict[str, Any],
    ) -> str:
        generic = {"", "scheduler_stalled", "execution_incomplete", "insufficient_step_budget"}
        skill = runtime.skill_run
        candidates = [
            skill.blocked_reason if skill is not None else "",
            runtime.blocked_reason,
            self._pending_runtime_reason(state, runtime),
        ]
        if runtime.domain == "meetingroom":
            bookings = (evidence.get("booking_query") or {}).get("bookings") or []
            if bookings and not evidence.get("selected_booking") and runtime.capability in {
                "meeting.cancel",
                "meeting.extend",
                "meeting.cancel_rebook",
                "meeting.rebook_larger",
            ...
            if "room_candidates" in evidence and not (evidence.get("room_candidates") or {}).get("rooms"):
                return "no_bookable_room"
        for reason in candidates:
            reason = str(reason or "")
            if reason not in generic:
                return reason
        return next((str(reason) for reason in candidates if reason), "scheduler_stalled")
    ...
    def _protect_specific_meeting_slots(self, current: dict[str, Any], incoming: dict[str, Any]) -> None:
        current_room_ids = [str(item) for item in current.get("room_ids") or [] if item]
        incoming_room_ids = [str(item) for item in incoming.get("room_ids") or [] if item]
        if current_room_ids:
            incoming["room_ids"] = self._dedupe([*current_room_ids, *incoming_room_ids])
    ...
    def _normalize_task_meeting_slots(self, slots: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in slots.items():
            ...
            elif target in {"start", "end"}:
                normalized_time = self._canonical_time_value(value)
                if normalized_time:
                    out[target] = normalized_time
    ...
    def _drain_read_plan(self, state: RuntimeState, llm_config: dict[str, Any]) -> bool:
        ...
        max_batch = min(self._parallel_read_max_batch_size(), max(0, state.step_budget - state.steps_used))
        if max_batch <= 0:
            return False
        ready_candidates = [
            task
            for task in plan.ready_tasks(
                state.read_task_keys_completed,
                max_batch,
                state.read_task_owner_keys_completed,
            )
    ...
    def _domain_remaining_step_estimate(self, state: RuntimeState, domain: str) -> dict[str, int]:
        runtimes = [
            runtime
            for runtime in state.task_runtimes
            if runtime.domain == domain
            and runtime.status not in TaskRuntime.TERMINAL_STATUSES
            and runtime.skill_run is not None
        ]
        reads = sum(runtime.skill_run.remaining_cost({"read", "postcheck"}) for runtime in runtimes)
        writes = sum(runtime.skill_run.remaining_cost({"write"}) for runtime in runtimes)
        return {"reads": reads, "writes": writes, "total": reads + writes}

    def _read_plan_step_reserve(self, state: RuntimeState) -> int:
        # A future write cannot be reached without its evidence reads. Reserving
        # that write here deadlocks the DAG when the remaining read is the only
        # operation that can unlock it.
        return 0

    def _build_read_plan(self, state: RuntimeState) -> ReadPlan:
        tasks: list[ReadTask] = []
        previous_active = dict(state.active_task_ids)
        transition_limit = max(1, sum(len(run.nodes) for run in state.skill_runs.values()))
        for _ in range(transition_limit):
            declarative_progress = False
            for runtime in state.task_runtimes:
                if runtime.status in TaskRuntime.TERMINAL_STATUSES or runtime.skill_run is None:
                    continue
                ...
                if (
                    ...
                    and runtime.intent == "expense_material"
                    and self._expense_missing_slot_name(state)
                ):
                    continue
                for node in runtime.skill_run.ready_nodes({"read", "postcheck"}):
                    handler = self.node_args_handlers.get(str(node.get("args_handler") or ""))
                    if handler is None:
                        runtime.skill_run.mark_blocked("missing_args_handler", str(node.get("id") or ""))
                        continue
                    directive = handler(state, runtime, node)
                    node_id = str(node.get("id") or "")
                    if directive.status in {"passed", "failed", "skipped"}:
                        validation = self._skill_validation(
                            "passed" if directive.status in {"passed", "skipped"} else "failed",
                            directive.reason,
                            directive.evidence_refs,
                            fingerprint_value={"task_id": runtime.task_id, "node_id": node_id},
                        )
                        transition = runtime.skill_run.apply_validation(node_id, validation)
                        declarative_progress = declarative_progress or transition.get("status") in {"completed", "skipped", "retry"}
                        continue
                    for action in directive.actions:
                        if not isinstance(action, StepAction) or action.kind != "tool" or action.tool not in READ_TOOLS:
                            continue
                        options = action.read_options if isinstance(action.read_options, dict) else {}
                        self._append_read_task(
                            ...
                            action.args,
                            runtime.domain,
                            parallel_eligible=bool(options.get("parallel_eligible", True)),
                            group_key=str(options.get("group_key") or ""),
                            stop_group_on_success=bool(options.get("stop_group_on_success")),
                            mapping_score=float(options.get("mapping_score") or 0.0),
                            owner_task_id=runtime.task_id,
                            owner_node_id=node_id,
                            owner_metadata=options.get("owner_metadata") if isinstance(options.get("owner_metadata"), dict) else {},
                        )
            if tasks or not declarative_progress:
                break
    ...
    def _append_read_task(
        ...
        owner_key = self._read_task_owner_key(task_key, resolved_owner_id, resolved_node_id)
        if owner_key in state.read_task_owner_keys_completed:
            return
        ...
        if owner is not None:
            ...
            if tool not in allowed_tools or not resolved_node_id:
                return
        existing = next((item for item in tasks if item.task_key == task_key), None)
        if existing is not None:
            existing.add_owner(resolved_owner_id, resolved_node_id, domain, owner_metadata)
            return
        tasks.append(
            ReadTask(
                task_key=task_key,
                ...
                owners=[
                    {
                        "task_id": resolved_owner_id,
                        "node_id": resolved_node_id,
                        "domain": domain,
                        "metadata": dict(owner_metadata or {}),
                    }
                ],
            )
        )
    ...
    def _append_project_search_fanout_tasks(self, state: RuntimeState, tasks: list[ReadTask], expense: dict[str, Any]) -> bool:
        ...
        if args.get("project_code"):
            ...
            return True
        variants = self._project_search_fanout_args(state, expense, args)[:2]
        appended = False
        ...
        for index, item in enumerate(variants):
            ...
            self._append_read_task(
                ...
                "workflow",
                group_key=group_key,
                # When the user supplied a formal name, keep one normalized
                # core query behind the original phrase before closing the group.
                stop_group_on_success=(len(variants) == 1 or index > 0) and self._project_query_quality(item) > 0.15,
    ...
    def _project_search_fanout_args(self, state: RuntimeState, expense: dict[str, Any], first_args: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        out: list[str] = []
        composed_candidates: set[str] = set()
        formal_core_candidates: set[str] = set()
        ...
        if explicit and not formal_explicit:
            for keyword in expense.get("project_keywords") or []:
                text = self._clean_project_phrase(keyword)
                if text and text != explicit:
                    composed = self._compose_project_business_candidate(text, explicit)
                    if composed:
                        out.append(composed)
                        composed_candidates.add(composed)
        if explicit:
            if formal_explicit:
                formal_cores = self._formal_hint_core_candidates(explicit)
                formal_core_candidates.update(formal_cores)
                out.extend(formal_cores)
        ...
        for keyword in expense.get("project_keywords") or []:
            ...
            if text:
                ...
                if formal_explicit or self._looks_like_formal_project_hint(text):
                    ...
                    formal_cores = self._formal_hint_core_candidates(text)
                    formal_core_candidates.update(formal_cores)
                    out.extend(formal_cores)
        ...
        if ordered:
            ...
            remaining_values = [
                ...
                for _, value in sorted(
                    ...
                    key=lambda pair: (
                        0 if pair[1] in formal_core_candidates else 1 if pair[1] in composed_candidates else 2,
                        -self._project_query_quality({"project_name": pair[1]}),
                        pair[0],
                    ),
                )
            ]
    ...
    def _compose_project_business_candidate(self, business_phrase: str, project_phrase: str) -> str:
        """Compose a bounded query from two user-provided phrases only."""
        business = re.sub(r"[^\u4e00-\u9fa5A-Za-z0-9]", "", str(business_phrase or ""))
        project = re.sub(r"[^\u4e00-\u9fa5A-Za-z0-9]", "", str(project_phrase or ""))
        if len(business) < 4 or len(project) < 4:
            return ""
        left = business[:2]
        right = project[:2]
        if left == right or left in project or right in business:
            return ""
        candidate = left + right
        return candidate if self._valid_project_search_candidate(candidate, allow_short=True) else ""
    ...
    def _append_project_empty_retry_tasks(self, state: RuntimeState, tasks: list[ReadTask], expense: dict[str, Any]) -> bool:
        ...
        if not llm_config.get("api_key") or not self._can_call_llm(state, "strong"):
            return False
    ...
    def _llm_project_search_mapping_variants(self, state: RuntimeState, expense: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        payload = {
            ...
            "instruction": (
                ...
                "Avoid exact failed args and amount/material/action-only words. Return only the ordered query strings without scores or explanations."
            ),
            "output_schema": {"queries": ["substring query"]},
        }
        try:
            content = self._chat_completion(
                llm_config,
                ...
                call_purpose="project_empty_retry_mapping",
            )
            parsed = self._parse_json_object(content) or {}
            raw_variants = parsed.get("queries") if isinstance(parsed.get("queries"), list) else []
            if not raw_variants and isinstance(parsed.get("variants"), list):
                raw_variants = parsed.get("variants") or []
            variants: list[dict[str, Any]] = []
            for index, raw in enumerate(raw_variants[:6]):
                ...
                if name and self._valid_project_search_candidate(name, allow_short=True):
                    variants.append({"project_name": name, "score": max(0.0, 1.0 - index * 0.1), "reason": "", "source": "llm"})
    ...
    @staticmethod
    def _read_task_owner_key(task_key: str, task_id: str, node_id: str) -> str:
        return f"{task_key}|{task_id or ''}|{node_id or ''}"
    ...
    def _read_task_allowed(self, state: RuntimeState, task: ReadTask) -> bool:
        if task.tool not in READ_TOOLS or task.tool not in state.tools:
            return False
        if isinstance(self.tool_registry, EffectiveToolRegistry) and not self.tool_registry.can_execute_read(task.tool):
            return False
        if not isinstance(self.tool_registry, EffectiveToolRegistry) and not self.tool_registry.is_read(task.tool):
            return False
        owner_keys = {
            self._read_task_owner_key(
                task.task_key,
        ...
        if owner_keys and owner_keys.issubset(state.read_task_owner_keys_completed):
            return False
    ...
    def _read_task_summary(self, task: ReadTask) -> dict[str, Any]:
        return {
            ...
            "attempt": int(state_attempt) if (state_attempt := getattr(task, "attempt", 0)) else 0,
        }
    ...
    def _execute_read_task(self, state: RuntimeState, task: ReadTask, llm_config: dict[str, Any]) -> None:
        ...
        previous_attempts = int(state.read_task_attempt_counts.get(task.task_key) or 0)
        attempt = previous_attempts or 1 if cached else previous_attempts + 1
        state.read_task_attempt_counts[task.task_key] = max(previous_attempts, attempt)
        state.read_task_keys_attempted.add(task.task_key)
        state.read_tasks_total += 1
        if not cached and attempt > 1:
            state.read_tasks_retried += 1
        ...
        context = ToolExecutionContext(
            ...
            domain=task.domain,
            attempt=attempt,
            metadata=dict((task.owners[0] if task.owners else {}).get("metadata") or {}),
        )
        outcome = self._execute(state, StepAction("tool", task.tool, task.args), llm_config, context)
        state.read_task_outcomes[task.task_key] = outcome
        if outcome.success:
            state.read_task_keys_completed.add(task.task_key)
            for item in task.owners:
                state.read_task_owner_keys_completed.add(
                    self._read_task_owner_key(
                        task.task_key,
                        str(item.get("task_id") or ""),
                        str(item.get("node_id") or ""),
            ...
            state.read_tasks_failed += 1
            self._ensure_primary_read_failure_recorded(state, task, outcome, context)
        self._fan_out_read_outcome(state, task, outcome, context)
        group_success = outcome.success and self._read_task_group_success(state, task)
        ...
        if not outcome.success and self._read_failure_retry_exhausted(state, task):
            state.read_retry_exhausted += 1
        self._debug_log(
            llm_config,
            {
                "event": "read_task",
    ...
    def _ensure_primary_read_failure_recorded(
        self,
        state: RuntimeState,
        task: ReadTask,
        ...
        owner = self._task_runtime(state, context.owner_task_id) if context.owner_task_id else None
        skill = owner.skill_run if owner is not None else None
        if skill is None or not context.owner_node_id:
            return
        fingerprint = self._skill_invocation_fingerprint(task.tool, task.args)
        if not skill.invocation_status(context.owner_node_id, fingerprint):
            self._record_skill_tool_outcome(state, task.tool, task.args, outcome.result, context)

    def _read_failure_retry_exhausted(self, state: RuntimeState, task: ReadTask) -> bool:
        owners = [
            self._task_runtime(state, str(item.get("task_id") or ""))
            for item in task.owners
            if item.get("task_id")
        ]
        return bool(owners) and all(
            owner is None
            or owner.status in TaskRuntime.TERMINAL_STATUSES
            or owner.skill_run is None
            or bool(owner.skill_run.blocked_reason)
            for owner in owners
        )

    def _fan_out_read_outcome(
        self,
        state: RuntimeState,
        task: ReadTask,
        outcome: ToolExecutionOutcome,
        primary_context: ToolExecutionContext,
    ) -> None:
        for item in task.owners:
            task_id = str(item.get("task_id") or "")
            node_id = str(item.get("node_id") or "")
            domain = str(item.get("domain") or task.domain or "")
            owner = self._task_runtime(state, task_id) if task_id else None
            if owner is None:
                continue
            self._activate_task_runtime_view(state, owner)
            node_result = {
            ...
            owner.local_evidence.setdefault("_node_results", {})[node_id] = node_result
            domain_state = state.meetingroom if domain == "meetingroom" else state.workflow
            domain_state.evidence.setdefault("_node_results", {})[node_id] = node_result
            if task_id == primary_context.owner_task_id and node_id == primary_context.owner_node_id:
                continue
            extra_context = ToolExecutionContext(
                owner_task_id=task_id,
                owner_node_id=node_id,
                domain=domain,
                attempt=primary_context.attempt,
                metadata=dict(item.get("metadata") or {}),
            )
            self._apply_tool_result(state, task.tool, task.args, outcome.result, extra_context)
    ...
    def _execute(
        ...
        owner = self._task_runtime(state, context.owner_task_id) if context and context.owner_task_id else None
        if owner is not None and owner.status not in TaskRuntime.TERMINAL_STATUSES:
            self._activate_task_runtime_view(state, owner)
        if action.kind == "block_meetingroom":
            ...
            override = action.args.get("result") if isinstance(action.args.get("result"), dict) else {}
            result = dict(override) if override else {"status": "blocked", "reason": state.meetingroom.blocked_reason}
            if action.args.get("order_id"):
                result.setdefault("order_id", action.args["order_id"])
            ...
            return ToolExecutionOutcome(success=True, result=result)
        ...
        if action.kind == "reply":
            if state.steps_used >= state.step_budget:
                result = {"error": "insufficient_step_budget"}
                return ToolExecutionOutcome(success=False, result=result, error_class="insufficient_step_budget")
            if not self._reply_available(state, action.message):
                domain = self._reply_domain(state)
                target = state.meetingroom if domain == "meetingroom" else state.workflow
                target.status = "blocked"
                target.blocked_reason = "reply_unavailable"
                target.result = {"status": "blocked", "reason": "reply_unavailable"}
                record = {
                    "tool": "__reply__",
                    "args": {"message": action.message},
                    "result": {"error": "reply_unavailable"},
                }
                self._annotate_action_record(state, record, action_started, bucket="program")
                state.history.append(record)
                self._debug_log(llm_config, {"event": "reply_blocked", **record})
                return ToolExecutionOutcome(success=False, result=record["result"], error_class="reply_unavailable")
            state.reply_messages_attempted.add(action.message)
            try:
            ...
            return ToolExecutionOutcome(
                ...
                error_class=self._tool_result_error_class(result),
                step_consumed=True,
            )
        ...
        if action.kind != "tool" or action.tool not in state.tools:
            ...
            return ToolExecutionOutcome(
                ...
                error_class="unauthorized_or_invalid_action",
            )
        if state.steps_used >= state.step_budget:
            result = {"error": "insufficient_step_budget"}
            return ToolExecutionOutcome(success=False, result=result, error_class="insufficient_step_budget")
        ...
        if not validation.get("passed"):
            record = {
                "tool": action.tool,
                "args": args,
                "result": {"error": "tool_contract_validation_failed", "validation": validation},
            }
            self._annotate_action_record(state, record, action_started, bucket="program")
            state.history.append(record)
            self._debug_log(llm_config, {"event": "invalid_action", **record})
            return ToolExecutionOutcome(
                success=False,
                result=record["result"],
                error_class="tool_contract_validation_failed",
            )
        if self.tool_registry.risk(action.tool) == "unknown":
            record = {
                "tool": action.tool,
                "args": args,
                "result": {"error": "unmapped_runtime_tool_not_executable"},
            }
            self._annotate_action_record(state, record, action_started, bucket="program")
            state.history.append(record)
            self._debug_log(llm_config, {"event": "invalid_action", **record})
            return ToolExecutionOutcome(
                success=False,
                result=record["result"],
                error_class="unmapped_runtime_tool_not_executable",
            )
        preflight_id = None
        if self.tool_registry.is_write(action.tool):
            action_domain = self._ledger_domain_for_tool(state, action.tool, context)
            ...
            if not preflight.get("passed"):
                ...
                self._mark_last_action_domain(state, action.tool, context)
                state.history.append(record)
        ...
        if cache_key and cache_key in state.cache:
            ...
            self._mark_last_action_domain(state, action.tool, context)
            state.history.append(record)
            self._apply_tool_result(state, action.tool, args, result, context)
            state.ledger.record_tool(
                tool=action.tool,
                args=args,
                result=result,
                kind="read",
                domain=self._ledger_domain_for_tool(state, action.tool, context),
                cached=True,
                preflight_id=preflight_id,
                owner_task_id=context.owner_task_id if context is not None else "",
                owner_node_id=context.owner_node_id if context is not None else "",
            )
            self._debug_log(llm_config, {"event": "tool_cache", **record})
            return ToolExecutionOutcome(success=True, result=result, cached=True)
        try:
    ...
    @staticmethod
    def _tool_result_error_class(result: Any) -> str:
    ...
    def _mark_last_action_domain(
        self,
        state: RuntimeState,
        tool: str,
        context: ToolExecutionContext | None = None,
    ) -> None:
        domain = self._ledger_domain_for_tool(state, tool, context)
        if domain in {"meetingroom", "workflow"}:
    ...
    def _ledger_domain_for_tool(
        self,
        state: RuntimeState,
        tool: str,
        context: ToolExecutionContext | None = None,
    ) -> str:
        if context is not None and context.domain in {"meetingroom", "workflow"}:
    ...
    def _apply_reply_result(self, state: RuntimeState, message: str, result: dict[str, Any]) -> None:
        ...
        if result.get("confirmed_action") == "meetingroom.booking.create":
            ...
            return
        if not resolved:
            state.reply_messages_unresolved.add(message)
            if action_slot := self._last_asked_slot(state):
                domain = self._reply_domain(state)
                target = state.meetingroom if domain == "meetingroom" else state.workflow
                target.evidence.setdefault("unresolved_slots", set()).add(action_slot)
                runtime = self._active_task_runtime(state, domain)
                if runtime is not None:
                    runtime.status = "blocked"
                    runtime.blocked_reason = "unresolved_user_input"
                    runtime.result = {
                        "status": "blocked",
                        "reason": "unresolved_user_input",
                        "slot": action_slot,
                    }
                    if runtime.skill_run is not None:
                        runtime.skill_run.mark_blocked("unresolved_user_input")
                        runtime.skill_run.terminal_result = dict(runtime.result)
            return
        ...
        elif resolved == "order_id":
            order_id = self._extract_order_id(text)
            if order_id:
                state.meetingroom.slots["order_id"] = order_id
    ...
    def _last_asked_slot(self, state: RuntimeState) -> str:
        for slot in ["order_id", "material_subclass", "material_category", "project_code", "total_amount", "approver", "reason", "leave_type", "end_time", "start_time", "title", "attendees", "day"]:
            if slot in state.asked_slots:
                return slot
        return ""

    def _reply_available(self, state: RuntimeState, message: str) -> bool:
        return bool(
            state.obs.get("mode") == "multi_turn"
            and callable(getattr(self.env, "reply", None))
            and message not in state.reply_messages_unresolved
        )

    def _reply_domain(self, state: RuntimeState) -> str:
        slot = self._last_asked_slot(state)
        if slot in {"order_id", "day", "attendees", "title"}:
            return "meetingroom"
        if slot:
            return "workflow"
    ...
    def _apply_tool_result(
        ...
        owner_domain = context.domain if context is not None else ""
        route_meetingroom = owner_domain == "meetingroom" or (
            not owner_domain
            and (
                tool.startswith("meetingroom.")
                or tool == "user.get_workspace"
                or (tool == "user.get_info" and state.meetingroom.intent.startswith("participant_"))
            )
        )
        route_workflow = owner_domain == "workflow" or (
            not owner_domain and (tool.startswith("workflow.") or tool.startswith("oa.") or tool in {"user.get_info", "file.list"})
        )
        if route_meetingroom:
            self._apply_meeting_tool_result(state, tool, args, result, context)
        if route_workflow:
            self._apply_workflow_tool_result(state, tool, args, result)
        if isinstance(result, dict) and not result.get("error"):
            state.completed_tools.add(tool)
        self._record_skill_tool_outcome(state, tool, args, result, context)
        if route_meetingroom:
            self._sync_meetingroom_skill(state)
        if route_workflow:
            self._sync_workflow_skill(state)
        if context is not None and context.owner_task_id:
            owner = self._task_runtime(state, context.owner_task_id)
            if owner is not None:
                self._capture_task_runtime_evidence(state, owner)

    def _record_skill_tool_outcome(
        self,
        state: RuntimeState,
        tool: str,
        args: dict[str, Any],
        ...
        if not isinstance(result, dict):
            return
        owner = self._task_runtime(state, context.owner_task_id) if context and context.owner_task_id else None
        skill = owner.skill_run if owner is not None else (
            self._domain_skill_run(state, "meetingroom") if tool.startswith("meetingroom.") else self._domain_skill_run(state, "workflow")
        )
        if tool == "user.get_info" and context is None:
            skill = self._domain_skill_run(state, "meetingroom") if state.meetingroom.intent.startswith("participant_") else self._domain_skill_run(state, "workflow")
        if skill is None:
            return
        node = skill.node(context.owner_node_id) if context and context.owner_node_id else next(
            (
                item
                for item in skill.nodes
                if skill.statuses.get(str(item.get("id") or "")) == "running"
                and tool in [str(item.get("tool") or ""), *[str(value) for value in item.get("tools") or []]]
            ),
            None,
        )
        if node is None:
        ...
        if result.get("error"):
            if tool == "workflow.project_search":
                reason = "project_search_error"
            elif tool == "workflow.search_person":
                reason = "approver_search_error"
            elif tool == "workflow.browser_search" and node_id == "subclass":
                reason = "subclass_search_error"
            elif tool.startswith("oa."):
                reason = "oa_lookup_error"
            else:
                reason = f"{node_id}_tool_error"
        elif tool == "workflow.project_search" and not (result.get("projects") or []):
            reason = "empty_project_result"
        elif tool == "workflow.search_person" and not (result.get("people") or []):
            reason = "empty_approver_result"
        elif tool == "workflow.browser_search" and not (result.get("options") or []):
            reason = "empty_subclass_result" if node_id == "subclass" else "empty_category_result"
        elif tool == "workflow.save" and not result.get("draft_saved"):
        ...
        elif tool in {"meetingroom.booking.participant.add", "meetingroom.booking.participant.remove"} and not (
    ...
    def _apply_meeting_tool_result(
        self,
        state: RuntimeState,
        tool: str,
        args: dict[str, Any],
        ...
        mr = state.meetingroom
        capability = self._active_task_capability(state, "meetingroom")
        ...
        if result.get("error"):
            return
        if tool == "user.get_info" and (
            (context is not None and context.owner_node_id == "resolve_people")
            or (context is None and mr.intent.startswith("participant_"))
        ):
            index = int((context.metadata if context is not None else {}).get("participant_index", mr.evidence.get("participant_index") or 0))
            mr.evidence[f"participant_user_{index}"] = result
        elif tool == "user.get_workspace":
            mr.evidence["workspace"] = result
            # Parallel room reads can select before workspace evidence arrives.
            mr.evidence.pop("pending_selected_room", None)
        ...
        elif tool in {"meetingroom.room.schedule", "meetingroom.room.bookings"}:
            if tool == "meetingroom.room.bookings":
                mr.evidence["room_occupancy"] = result
            mr.evidence.setdefault("schedules", {})[args.get("room_id")] = result
            if capability not in {"meeting.query_room_schedule", "query_room_schedule"} and args.get("room_id"):
                room = dict(self.meetingroom_index.room(args.get("room_id")) or {})
                if room:
                    room.setdefault("room_id", args.get("room_id"))
                    room.setdefault("busy_slots", [])
                    room.setdefault("bookable", True)
                    mr.evidence["room_candidates"] = self._merge_room_list_results(
                        mr.evidence.get("room_candidates"),
                        {"day": self._meeting_day(state), "rooms": [room]},
                    )
            ...
            if bookings:
                ...
                if self._is_rebook_larger_intent(capability) and mr.evidence["selected_booking"]:
                    selected = mr.evidence["selected_booking"]
                    for key in ("day", "start", "end", "title"):
                        if selected.get(key) not in (None, ""):
                            mr.slots[key] = selected[key]
        elif tool == "meetingroom.booking.cancel" and result.get("cancelled"):
            mr.evidence["cancel_done"] = result
            if self._is_cancel_intent(capability):
                mr.status = "done"
            ...
            if not result.get("error") and (result.get("success") or result.get("added") or result.get("removed")):
                index = int(mr.evidence.get("participant_index") or 0)
    ...
    def _apply_workflow_tool_result(self, state: RuntimeState, tool: str, args: dict[str, Any], result: dict[str, Any]) -> None:
        ...
        capability = self._active_task_capability(state, "workflow")
        if tool == "file.list":
            wf.evidence["file_list_attempted"] = {"args": dict(args), "result": result}
        ...
        if state.task_runtimes:
            for runtime in state.task_runtimes:
                if runtime.status in TaskRuntime.TERMINAL_STATUSES:
                    continue
                reason = runtime.blocked_reason or self._pending_runtime_reason(state, runtime)
                if runtime.skill_run is not None:
            ...
            for domain in ("meetingroom", "workflow"):
                ...
                if domain_state.status == "blocked":
                    domain_state.blocked_reason = next(
                        (runtime.blocked_reason for runtime in domain_tasks if runtime.blocked_reason),
                        "scheduler_stalled",
                    )
            return
        if state.meetingroom.needed and state.meetingroom.status in {"pending", "ready"}:
    ...
    def _pending_runtime_reason(self, state: RuntimeState, runtime: TaskRuntime) -> str:
        skill = runtime.skill_run
        if skill is not None:
            if skill.terminal_result.get("reason"):
                return str(skill.terminal_result["reason"])
            if skill.blocked_reason:
                return str(skill.blocked_reason)
            if skill.failures:
                reason = str(skill.failures[-1].get("reason") or "")
                if reason:
                    return reason
        evidence = state.meetingroom.evidence if runtime.domain == "meetingroom" else state.workflow.evidence
        if evidence.get("unresolved_slots"):
            return "unresolved_user_input"
        if runtime.domain == "meetingroom":
            if "room_candidates" in evidence and not (evidence.get("room_candidates") or {}).get("rooms"):
                return "no_bookable_room"
            if "booking_query" in evidence and not (evidence.get("booking_query") or {}).get("bookings"):
                return "existing_booking_not_found"
        else:
            if evidence.get("project_search_history") and not evidence.get("verified_project"):
                return "ambiguous_project"
            if evidence.get("subclass_options") and not evidence.get("workflow_skill_draft_ir"):
                return "ambiguous_material_subclass"
        if state.steps_used >= state.step_budget:
            return "insufficient_step_budget"
        return "scheduler_stalled"

    def _block_scheduler_stall(
        self,
        state: RuntimeState,
        diagnostic: dict[str, Any],
        llm_config: dict[str, Any],
    ) -> None:
        reason = str(diagnostic.get("reason") or "scheduler_stalled")
        state.scheduler_stalls.append(dict(diagnostic))
        self._debug_log(
            llm_config,
            {
                "event": "scheduler_stalled",
                "case_id": state.obs.get("case_id"),
                **diagnostic,
            },
        )
        for runtime in state.task_runtimes:
            ...
            if runtime.skill_run is not None:
                runtime.skill_run.mark_blocked(reason)
                if not runtime.skill_run.terminal_result:
                    runtime.skill_run.terminal_result = {
                        "status": "blocked",
                        "reason": reason,
    ...
    def _is_cancel_intent(self, intent: str) -> bool:
        return str(intent or "") in {"cancel", "cancel_existing", "meeting.cancel"}

    def _is_extend_intent(self, intent: str) -> bool:
        return str(intent or "") in {"extend", "extend_existing", "meeting.extend"}

    def _is_rebook_larger_intent(self, intent: str) -> bool:
        return str(intent or "") in {"rebook_larger", "rebook_larger_existing", "meeting.rebook_larger"}

    def _is_cancel_rebook_intent(self, intent: str) -> bool:
        return str(intent or "") in {"cancel_rebook", "cancel_rebook_existing", "meeting.cancel_rebook"}
    ...
    def _normalize_meeting_slots(self, state: RuntimeState) -> None:
        ...
        if slots.get("multi_segments"):
            ...
            segments = self._normalize_multi_segments(
                state,
                slots.get("segments") or self._extract_meeting_segments(query),
            )
    ...
    def _normalize_multi_segments(self, state: RuntimeState, raw_segments: Any) -> list[dict[str, str]]:
        ...
        for raw in raw_segments:
            ...
            if not start or not end or self._minutes_between(start, end) <= 0:
                continue
            day_text = str(raw.get("day_text") or raw.get("date_ref") or raw.get("day") or default_day_text)
    ...
    def _room_list_args(self, state: RuntimeState, day: str | None = None) -> dict[str, Any] | None:
        ...
        if not day_value:
            return None
        args: dict[str, Any] = {"capacity_gte": int(slots.get("capacity") or 10), "bookable": True}
        args["day"] = day_value
    ...
    def _room_search_candidates(self, state: RuntimeState) -> list[dict[str, str]]:
        ...
        prefer_address = bool(slots.get("office_address_candidates")) and (
            ...
            or any(word in self._full_query(state.obs) for word in ["楼", "层", "小镇", "合肥"])
        )
        if prefer_address:
    ...
    def _select_room_for_booking(self, state: RuntimeState) -> dict[str, Any] | None:
        ...
        for room in rooms:
            if not room.get("bookable", True):
                continue
            if self._is_rebook_larger_intent(capability) and mr.evidence.get("selected_booking"):
        ...
        if self._is_rebook_intent(capability) and mr.evidence.get("selected_booking"):
    ...
    def _booking_create_args(self, state: RuntimeState, room: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "day": day,
            "office_id": room.get("officeId") or room.get("office_id") or self.meetingroom_index.office_id_for_room(room_id) or room.get("building") or mr.slots.get("office_candidates", [""])[0],
            "room_id": room_id,
            "start": mr.slots.get("start"),
            "end": mr.slots.get("end"),
            "title": self._normalize_meeting_title(mr.slots.get("title") or "项目复盘"),
    ...
    def _booking_day_from_schedule_context(self, state: RuntimeState) -> str:
        query_day = self._resolve_day(self._extract_day_text(self._meeting_query(state)), state.obs.get("now"))
        if query_day:
    ...
    def _next_booking_list_args(self, state: RuntimeState) -> dict[str, Any] | None:
        mr = state.meetingroom
        capability = self._active_task_capability(state, "meetingroom")
        query_only = capability == "meeting.query_booking" or capability in {"query_booking", "query"}
        ...
        for day in self._meeting_day_candidates(state):
            ...
            if day:
                args["day"] = day
            if keyword and not (self._is_extend_intent(capability) or self._is_rebook_intent(capability)):
                args["keyword"] = keyword
            if args not in tried:
                return args
        args = {} if query_only else {"status": "active"}
        if keyword and not (self._is_extend_intent(capability) or self._is_rebook_intent(capability)):
            args["keyword"] = keyword
        if args not in tried:
            return args
    ...
    def _canonical_selected_booking_list_args(self, state: RuntimeState) -> dict[str, Any] | None:
        mr = state.meetingroom
        ...
        if not day:
            return None
        args: dict[str, Any] = {"status": "active", "day": day}
        keyword = self._normalize_meeting_title(booking.get("title"))
        if keyword:
            args["keyword"] = keyword
        tried = [
            item.get("args")
            for item in mr.evidence.get("tried_booking_lists", [])
            if isinstance(item, dict) and isinstance(item.get("args"), dict)
        ]
        return None if args in tried else args
    ...
    def _extension_has_known_conflict(self, state: RuntimeState, booking: dict[str, Any]) -> bool:
        ...
        occupancy = state.meetingroom.evidence.get("room_occupancy") or {}
        for other in occupancy.get("bookings") or []:
            other_id = other.get("order_id") or other.get("booking_id")
            if other_id == booking_id or other.get("status") == "cancelled":
                continue
            if self._time_overlap(str(booking.get("end") or ""), new_end, str(other.get("start") or ""), str(other.get("end") or "")):
                return True
    ...
    def _requires_room_occupancy_read(self, state: RuntimeState) -> bool:
        mr = state.meetingroom
        capability = self._active_task_capability(state, "meetingroom")
        if not (self._is_extend_intent(capability) or self._is_cancel_rebook_intent(capability)):
            return False
        query = self._meeting_query(state)
        return str(mr.slots.get("fallback_policy") or "") == "keep_if_extend_conflict" or any(
            word in query for word in ["冲突", "别动", "保持原样", "先别动"]
        )
    ...
    def _booking_result_from_create(self, args: dict[str, Any], result: dict[str, Any], mr: DomainState) -> dict[str, Any]:
        ...
        if self._is_rebook_intent(mr.intent) and mr.evidence.get("cancel_done") and mr.evidence.get("selected_booking"):
    ...
    def _display_office_id(self, office_id: Any, room_id: Any, mr: DomainState) -> str:
        ...
        for room in candidates:
            if room.get("room_id") == room_id:
                if mr.slots.get("needs_workspace") or re.match(r"^A\d-", str(room_id or ""), flags=re.I):
                    return str(room.get("building") or office_id or "")
    ...
    def _select_booking(self, state: RuntimeState, bookings: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if selected.get("attendees") and int(mr.slots.get("capacity") or 0) <= int(selected.get("attendees") or 0) and self._is_rebook_larger_intent(capability):
    ...
    def _existing_booking_write_requires_confirmation(self, state: RuntimeState) -> bool:
        ...
        if self._extract_order_id(query) or mr.slots.get("order_id"):
            return False
        if self._is_cancel_rebook_intent(capability) and any(word in query for word in ["延", "重订", "重新预订", "不行就"]):
            return False
    ...
    def _booking_list_requires_confirmation_before_write(self, state: RuntimeState, bookings: list[dict[str, Any]]) -> bool:
        mr = state.meetingroom
        capability = self._active_task_capability(state, "meetingroom")
        if not (self._is_cancel_intent(capability) or self._is_cancel_rebook_intent(capability)):
            return False
        query = self._meeting_query(state)
        if self._extract_order_id(query) or mr.slots.get("order_id"):
            return False
    ...
    def _meeting_day_candidates(self, state: RuntimeState) -> list[str]:
        ...
        prefer_workday = self._is_meeting_context(query) and any(token in day_text for token in ("明天", "后天"))
        primary = self._resolve_day(day_text, state.obs.get("now"), prefer_workday=prefer_workday)
        candidates: list[str] = []

        if primary:
            candidates.append(primary)
        if slots.get("day") and not primary:
    ...
    def _schedule_range(self, state: RuntimeState) -> tuple[str, str]:
        ...
        if len(dates) >= 2:
            return self._resolve_day(dates[0], now), self._resolve_day(dates[1], now)
        shortened_range = re.search(
            r"(\d{1,2})\s*月\s*(\d{1,2})\s*日\s*(?:到|至|-|~)\s*(\d{1,2})\s*日",
            day_text,
        )
        if shortened_range:
            month, start_day, end_day = shortened_range.groups()
            return self._resolve_day(f"{month}月{start_day}日", now), self._resolve_day(f"{month}月{end_day}日", now)
        numbered_range = re.search(r"(\d{1,2})号\s*(?:到|至|-|~)\s*(\d{1,2})号", day_text)
        if numbered_range:
    ...
    def _normalize_workflow_slots(self, state: RuntimeState) -> None:
        ...
        if explicit_submit:
            ...
            wf.slots["submit"] = self._default_leave_submit_intent(query)
        else:
    ...
    def _leave_plan(self, state: RuntimeState) -> dict[str, Any] | None:
        ...
        explicit_start, explicit_end = self._extract_time_range(leave_text)
        explicit_leave_days = self._extract_requested_leave_days(leave_text)
        spans_multiple_days = bool(end_day and start_day and end_day != start_day)
        if duration and "下午" in leave_text and not (explicit_start or explicit_end):
            start = "14:00"
            end = self._add_minutes(start, int(float(duration) * 60))
        elif not start and duration and "下午" in query:
            start = "14:00"
    ...
    def _resolve_leave_date_range(self, day_text: Any, state: RuntimeState) -> tuple[str, str]:
        ...
        weekday_range = re.search(
            r"(下周|本周|周)?([一二三四五六日天])\s*(?:到|至|-|~)\s*(?:(下周|本周|周))?([一二三四五六日天])",
            text,
        )
        if weekday_range:
            start_prefix = weekday_range.group(1) or "周"
            end_prefix = weekday_range.group(3) or weekday_range.group(1) or "周"
            start = self._resolve_day(f"{start_prefix}{weekday_range.group(2)}", state.obs.get("now"))
            end = self._resolve_day(f"{end_prefix}{weekday_range.group(4)}", state.obs.get("now"))
            return start, end
    ...
    def _leave_plans(
        ...
        plan = plan or self._leave_plan(state)
        if not plan:
            return []
        query = self._full_query(state.obs)
        leave = self._leave_slots(state)
    ...
    def _leave_attachment_directory(self, state: RuntimeState) -> str:
        ...
        if not replacement_needs_document and not any(word in query for word in ["附件", "上传", "目录", "文件", "证明", "病假条"]):
            return ""
        match = re.search(r"([A-Za-z0-9_./-]+)\s*目录", query)
    ...
    def _next_approver_search_args(
        ...
        *,
        ...
        tried = state.workflow.evidence.get("approver_search_tried") or []
        if mark_attempt and "approver_search_tried" not in state.workflow.evidence:
            state.workflow.evidence["approver_search_tried"] = tried
        leave = self._leave_slots(state)
    ...
    def _next_project_search_args(self, state: RuntimeState, expense: dict[str, Any], allow_llm: bool = True) -> dict[str, Any] | None:
        ...
        strong_available = (
            ...
            and self._can_call_llm(state, "strong")
        )
    ...
    def _disambiguating_project_search_args(
        self,
        state: RuntimeState,
        ...
        for project in projects:
            ...
            candidates = self._dedupe(
                [
                    *self._project_candidate_forms(project_name),
                    *self._formal_hint_core_candidates(project_name),
                ]
            )
            for candidate in candidates:
    ...
    def _project_search_args_from_llm(self, state: RuntimeState, expense: dict[str, Any], force: bool = False) -> dict[str, Any] | None:
        ...
        if project_attempts >= 4 or remaining_steps <= 3:
            return None
        if not self._can_call_llm(state, "strong"):
            return None
    ...
    def _project_refine_search_args(self, state: RuntimeState, project: dict[str, Any]) -> dict[str, Any] | None:
        ...
        expense_tried = self._expense_slots(state).setdefault("_tried_project_keywords", [])
        for query in tried:
    ...
    def _llm_project_keyword_suggestions(self, state: RuntimeState, expense: dict[str, Any]) -> list[str]:
        ...
        payload = {
            ...
            "instruction": (
                ...
                "Return only the ordered query strings without reasons."
            ),
            "output_schema": {"queries": ["project search keyword"]},
        }
        try:
            content = self._chat_completion(
                llm_config,
                ...
                call_purpose="project_keyword_suggestions",
            )
            parsed = self._parse_json_object(content) or {}
            raw_keywords = parsed.get("queries") if isinstance(parsed.get("queries"), list) else []
            if not raw_keywords and isinstance(parsed.get("keywords"), list):
    ...
    def _select_material_category(self, state: RuntimeState) -> dict[str, Any] | None:
        ...
        for literal_hint in literal_sources:
            ...
            if literal:
                ...
                return literal
        if len(options) > 1 and self._can_call_llm(state, "strong"):
    ...
    def _expense_ir_save_args_or_block(
        ...
        llm_config = self._llm_config("strong")
        if llm_config.get("api_key") and self._can_call_llm(state, "strong"):
    ...
    def _llm_expense_draft_ir(
        ...
        payload = {
            ...
            "instruction": (
                "Return valid json only. Translate the user expense into detail rows. Use only listed subclass candidate ids. "
                ...
                "If the user only gave a category, purpose, or total budget without a concrete line item and allocation, return an empty details array. "
                ...
                "Return only the details field without decisions, confidence, missing fields, or explanations."
            ),
            "output_schema": {
                "details": [
                    {
        ...
        try:
            content = self._chat_completion(
                llm_config,
                [
                    {"role": "system", "content": "Return valid json only. Return exactly one details array for the constrained expense translation."},
                ...
                call_purpose="expense_detail_translation",
            )
            parsed = self._parse_json_object(content)
            if isinstance(parsed, dict):
                if "decision" not in parsed:
                    parsed["decision"] = "draft" if isinstance(parsed.get("details"), list) and parsed.get("details") else "need_more_info"
    ...
    def _llm_workflow_form_draft(self, state: RuntimeState, llm_config: dict[str, Any], snapshot: dict[str, Any]) -> dict[str, Any] | None:
        ...
        payload = {
            ...
            "instruction": (
                ...
                "If no subclass option is semantically relevant, return an empty details array. "
                "Return only total_amount and details without decisions, confidence, missing fields, or explanations."
            ),
            "output_schema": {
        ...
        try:
            content = self._chat_completion(
                ...
                call_purpose="workflow_form_draft",
            )
            parsed = self._parse_json_object(content)
            if isinstance(parsed, dict):
    ...
    def _specific_material_name(self, item_name: Any, expense: dict[str, Any], option: dict[str, Any] | None = None) -> str:
        ...
        cleaned_item = self._clean_material_name_match(item_text, item_text) if item_text else ""
        normalized_item = self._normalize_option_text(cleaned_item)
        normalized_label = self._normalize_option_text(option_label)
        if normalized_item and normalized_label and normalized_label in normalized_item:
            return option_label
        if cleaned_item and not self._looks_like_invalid_expense_item_name(cleaned_item):
            return cleaned_item
    ...
    def _has_unallocated_multi_material_budget(self, expense: dict[str, Any], options: list[dict[str, Any]]) -> bool:
        ...
        if any(label and (normalized_name == label or normalized_name in label) for label in labels):
            return False
        parts = [part.strip("，。；;、 的") for part in re.split(r"[、和与]|(?<!其)及", name) if part.strip("，。；;、 的")]
        if len(parts) > 1:
            mapped_ids: list[str] = []
            for part in parts:
                ...
                if not selected:
                    mapped_ids = []
                    break
                mapped_ids.append(str(selected.get("value") or selected.get("code") or ""))
            if mapped_ids and len(set(mapped_ids)) == 1:
                return False
    ...
    def _can_reuse_subclass_option(self, hint: str, option: dict[str, Any]) -> bool:
        # Several explicit rows may legitimately belong to one browser option
        # (for example, a monitor and a dock are both computer accessories).
        return True
    ...
    def _meeting_query(self, state: RuntimeState) -> str:
        source = state.meetingroom.slots.get("source_text")
        if source:
            return self._domain_source_text(str(source), "meetingroom", state.meetingroom.intent)
        return self._domain_source_text(
            self._task_source_text(state, "meetingroom", state.meetingroom.intent),
            "meetingroom",
            state.meetingroom.intent,
        )
    ...
    def _rank_candidates_with_llm(
        ...
        if not candidates:
            return {"decision": "need_more_info", "selected_id": ""}
        if len(candidates) == 1:
            key = next(iter(self._candidate_map(candidates, id_fields)))
            return {"decision": "select", "selected_id": key}
        llm_config = self._llm_config("strong")
        if not llm_config.get("api_key") or not self._can_call_llm(state, "strong"):
            return {"decision": "need_more_info", "selected_id": ""}
        by_key = self._candidate_map(candidates, id_fields)
        ...
        payload = {
            ...
            "candidates": compact,
            "instruction": (
                "Return valid json only. Select a candidate only when the user's wording uniquely supports it. "
                "Use an existing candidate id, or null when multiple candidates remain plausible. "
                "Do not return confidence, ranking, reasons, or any candidate fields."
            ),
            "output_schema": {"selected_id": "existing candidate id or null"},
        }
        try:
            content = self._chat_completion(
                llm_config,
                [
                    {
                        "role": "system",
                        "content": (
                            "Return valid json only. Select one supplied candidate id or null. "
                            "Never invent ids or business facts. Return exactly the selected_id field."
                        ),
                    },
                    {"role": "user", "content": "Return valid json only.\n" + json.dumps(payload, ensure_ascii=False)},
                ],
            ...
            if selected_id not in by_key:
                selected_id = ""
            decision = "select" if selected_id else "need_more_info"
            result = {
                "decision": decision,
                "selected_id": selected_id,
            }
    ...
    def _apply_participant_list_evidence(self, state: RuntimeState, result: dict[str, Any]) -> None:
        mr = state.meetingroom
        capability = self._active_task_capability(state, "meetingroom")
        if capability in {"meeting.participant_list", "participant_list"}:
            participants = result.get("participants") if isinstance(result.get("participants"), list) else []
            mr.status = "done"
            mr.result = {"status": "queried", "order_id": result.get("order_id"), "participants": participants}
            return
        if capability not in {"meeting.participant_add", "participant_add"} or not self._participant_duplicate_check_required(state):
            return
        participants = mr.slots.get("participants") if isinstance(mr.slots.get("participants"), list) else []
        index = int(mr.evidence.get("participant_index") or 0)
    ...
    def _build_final_answer(self, state: RuntimeState) -> dict[str, Any]:
        ...
        for domain in ("meetingroom", "workflow"):
            fields = [
                key
                for key in answer
                if (domain == "meetingroom" and key in {"booking_result", "participant_result", "participants", "participants_added"})
    ...
    def _all_done(self, state: RuntimeState) -> bool:
        ...
        if state.llm_semantic.get("semantic_error"):
            return True
        domains = []
        if state.meetingroom.needed:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if re.fullmatch(r"周[一二三四五六日天]", value):
                    ...
                    if "下周" in prefix_text:
                        return f"下周{value[-1]}"
                    if "本周" in prefix_text:
                        return f"本周{value[-1]}"
                return value
        return ""
    ...
    def _extract_leave_day_text(self, text: str) -> str:
        ...
        if weekday_range:
            return weekday_range.group(0)
        date_matches = list(re.finditer(r"\d{1,2}\s*月\s*\d{1,2}\s*日", text))
        if len(date_matches) >= 2:
            return text[date_matches[0].start() : date_matches[1].end()]
        patterns = [
            r"明天.*?到\s*后天",
            r"今天.*?到\s*明天",
            r"后天.*?到\s*大后天",
    ...
    def _leave_submit_intent(self, query: str) -> bool:
        if re.search(r"(?:直接)?提交[^，。；;]{0,10}(?:请假|事假|年假|病假|育儿假|婚假|陪产假|丧假)(?:申请)?", query):
            return True
        return any(
            word in query
            for word in [
    ...
    def _default_leave_submit_intent(self, query: str) -> bool:
        if self._draft_intent(query):
            return False
        if self._leave_submit_intent(query):
            return True
        return any(leave_type in query for leave_type in ["婚假", "陪产假", "丧假"])

    def _extract_requested_leave_days(self, text: str) -> int:
        match = re.search(r"请\s*(\d+|[一二两三四五六七八九十]+)\s*天", str(text or ""))
        if not match:
            return 0
        token = match.group(1)
    ...
    def _extract_contextual_time_range(self, text: str) -> tuple[str, str]:
        pattern = (
            ...
            r"(?P<h1>\d{1,2}|[一二两三四五六七八九十]+)\s*点(?:(?P<half1>半)|(?P<m1>\d{1,2})分?)?"
            r".{0,12}?(?:到|至|-|~)"
            r".{0,12}?"
            r"(?P<p2>上午|下午|中午|早上|晚上)?\s*"
            r"(?P<h2>\d{1,2}|[一二两三四五六七八九十]+)\s*点(?:(?P<half2>半)|(?P<m2>\d{1,2})分?)?"
        )
        match = re.search(pattern, text)
        if not match:
            return "", ""
        ...
        m1 = 30 if match.group("half1") else int(match.group("m1") or 0)
        m2 = 30 if match.group("half2") else int(match.group("m2") or 0)
        return f"{h1:02d}:{m1:02d}", f"{h2:02d}:{m2:02d}"
    ...
    def _extract_room_ids(self, text: str) -> list[str]:
        room_ids = [match.group(0).upper() for match in re.finditer(r"A\d-\dF-\d{3}", text, flags=re.I)]
        for match in re.finditer(r"(?<![A-Za-z0-9])A(\d)-(\d{3})(?![A-Za-z0-9])", text, flags=re.I):
            building, room_number = match.groups()
            normalized = f"A{building}-{room_number[0]}F-{room_number}".upper()
            if normalized not in room_ids:
                room_ids.append(normalized)
        return room_ids

    def _office_id_from_room_id(self, room_id: Any) -> str:
        return str(self.meetingroom_index.office_id_for_room(room_id) or "")
    ...
    def _normalize_meeting_title(self, title: Any) -> str:
        ...
        value = re.sub(r"^(?:主题)?还是", "", value).strip("，。,. ")
    ...
    def _extract_participants(self, text: str) -> list[dict[str, str]]:
        ...
        def add_person(raw_name: str, raw_employee_no: str = "") -> None:
            ...
            if name:
                item["name"] = name
            id_key = "user_id" if employee_no and len(employee_no) <= 6 else "employee_no"
            if employee_no:
                item[id_key] = employee_no
                seen_numbers.add(employee_no)
            if name:
                existing = by_name.get(name)
                if existing:
                    if employee_no and not existing.get(id_key):
                        existing[id_key] = employee_no
                    return
                by_name[name] = item
            people.append(item)
    ...
    def _extract_approver_hints(self, text: str) -> dict[str, str]:
        ...
        if not raw:
            raw = self._first_regex(
                text,
                r"审批人(?:必须是|要是|指定为|定为|选|找|是|还是|仍是|改成|改为|换成|换为)?([^，。；;]{1,16})",
            )
    ...
    def _slice_workflow_text(self, query: str, kind: str) -> str:
        if kind == "leave":
            ...
            connector = max(query.rfind(word, 0, trigger_pos) for word in ["另外", "顺手", "同时", "然后", "还有", "再帮"])
            start = connector + len(next((word for word in ["另外", "顺手", "同时", "然后", "还有", "再帮"] if query.startswith(word, connector)), "")) if connector >= 0 else max(0, trigger_pos - 40)
            return query[start:]

        expense_keys = ["费用", "采购", "报销", "物资", "设备申请"]
        ...
        if boundary >= 0:
            ...
            connector_words = ["另外", "顺手", "同时", "然后", "还有", "再帮"]
            connector_positions = [(query.rfind(word, 0, trigger_pos), word) for word in connector_words]
            connector_pos, connector_word = max(connector_positions, key=lambda item: item[0])
            start = connector_pos + len(connector_word) if connector_pos >= 0 else max(0, trigger_pos - 40)
        else:
            start = 0
        return query[start:]
    ...
    def _extract_expense_items(self, text: str) -> list[dict[str, Any]]:
        ...
        if specific and (not items or self._expense_item_name_quality([specific]) > self._expense_item_name_quality(items)):
            items = [specific]
        if not items:
    ...
    def _clean_explicit_expense_items(self, rows: list[Any]) -> list[dict[str, Any]]:
        ...
        for index, row in enumerate(rows):
            ...
            raw_budget = item.get("budget_amount") or item.get("amount")
            budget = self._money(raw_budget) if raw_budget else ""
    ...
    def _extract_specific_item_with_total_budget(self, text: str) -> dict[str, Any]:
        ...
        patterns = [
            (1, r"(?:要做|需要|采购|购买|要买|包括|包含)(?:一批|一些)?\s*([^，。；;、]{2,18}?)(?:，|。|；|;|预算|总预算|总金额|金额)"),
            (2, r"(?:(?:存|保存|提交|申请)(?:一个|一笔|个)?|(?:一个|一笔|个))\s*([^，。；;、]{2,18}?)(?:采购|费用)?草稿"),
        ]
        for evidence_rank, pattern in patterns:
            for match in re.finditer(pattern, str(text or "")):
                ...
                if name and not self._looks_like_invalid_expense_item_name(name):
                    candidates.append((evidence_rank, -match.start(), name))
        if not candidates:
            return {}
        _, _, name = max(candidates, key=lambda item: (item[0], item[1], len(item[2])))
        return {"name": name, "quantity": "1", "unit_price": amount, "budget_amount": amount}
    ...
    def _workflow_option_value(
        ...
        *,
        ...
        if len(options) == 1:
            return str(options[0].get("value") or options[0].get("code") or "")
    ...
    def _canonical_time_value(self, value: Any) -> str:
        ...
        if re.fullmatch(r"\d{1,2}:\d{1,2}", text):
            ...
            return f"{hour:02d}:{minute:02d}"
        if re.search(r"(?:\d{1,2}|[一二两三四五六七八九十]+)点", text):
    ...
    def _schedule_required_days(self, state: RuntimeState) -> list[str]:
        query = self._meeting_query(state)
        now = state.obs.get("now")
        selected_booking = state.meetingroom.evidence.get("selected_booking")
        if self._is_rebook_larger_intent(self._active_task_capability(state, "meetingroom")) and isinstance(selected_booking, dict):
            selected_day = str(selected_booking.get("day") or "")
            if selected_day:
                return [selected_day]
        days = [
            str(item.get("day") or "")
